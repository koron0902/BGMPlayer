...
#include "opus/libopus/include/opus_multistream.h"
#include <stdarg.h>
#include <stdio.h>
...
op_open_callbacks(void *_source, const OpusFileCallbacks *_cb,
...
/**Partially open a stream using the given set of callbacks to access it.
   ...
   \return A partially opened \c OggOpusFile, or <code>NULL</code> on error.
           ...
           The calling application is responsible for closing the source if
            this call returns an error.*/
OP_WARN_UNUSED_RESULT OggOpusFile *
op_test_callbacks(void *_source, const OpusFileCallbacks *_cb,
                  const unsigned char *_initial_data, size_t _initial_bytes,
                  int *_error) OP_ARG_NONNULL(2);
