...
static opus_uint32 op_parse_uint32le(const unsigned char *_data) {
  return _data[0] | (opus_uint32)_data[1] << 8 | (opus_uint32)_data[2] << 16 |
         (opus_uint32)_data[3] << 24;
}

static opus_uint32 op_parse_uint32be(const unsigned char *_data) {
  return _data[3] | (opus_uint32)_data[2] << 8 | (opus_uint32)_data[1] << 16 |
         (opus_uint32)_data[0] << 24;
}
...
int opus_tagncompare(const char *_tag_name, int _tag_len,
                     const char *_comment) {
  int ret;
...
const char *opus_tags_query(const OpusTags *_tags, const char *_tag,
                            int _count) {
  char **user_comments;
  int tag_len;
  int found;
...
static void op_extract_jpeg_params(const unsigned char *_buf, size_t _buf_sz,
                                   ...
                                   opus_uint32 *_depth, opus_uint32 *_colors,
                                   int *_has_palette) {
  if (op_is_jpeg(_buf, _buf_sz)) {
    ...
    for (;;) {
      ...
      if (segment_len < 2 || _buf_sz - offs < segment_len)
        break;
      if (marker == 0xC0 ||
          (marker > 0xC0 && marker < 0xD0 && (marker & 3) != 0)) {
...
static int op_is_gif(const unsigned char *_buf, size_t _buf_sz) {
  return _buf_sz >= 6 &&
         (memcmp(_buf, "GIF87a", 6) == 0 || memcmp(_buf, "GIF89a", 6) == 0);
}
...
static int opus_picture_tag_parse_impl(OpusPictureTag *_pic, const char *_tag,
                                       unsigned char *_buf, size_t _buf_sz,
                                       size_t _base64_sz) {
  ...
  if (mime_type_length > _buf_sz - 32)
    return OP_ENOTFORMAT;
  mime_type =
      (char *)_ogg_malloc(sizeof(*_pic->mime_type) * (mime_type_length + 1));
  if (mime_type == NULL)
    return OP_EFAULT;
  ...
  if (mime_type_length == 3 && strcmp(mime_type, "-->") == 0) {
    ...
    if (picture_type == 1 && (width != 0 || height != 0) &&
        (width != 32 || height != 32)) {
      return OP_ENOTFORMAT;
    }
    ...
    case OP_PIC_FORMAT_JPEG: {
      op_extract_jpeg_params(_buf + i, data_length, &file_width, &file_height,
                             &file_depth, &file_colors, &has_palette);
    } break;
    case OP_PIC_FORMAT_PNG: {
      op_extract_png_params(_buf + i, data_length, &file_width, &file_height,
                            &file_depth, &file_colors, &has_palette);
    } break;
    case OP_PIC_FORMAT_GIF: {
      op_extract_gif_params(_buf + i, data_length, &file_width, &file_height,
                            &file_depth, &file_colors, &has_palette);
    } break;
    }
    if (has_palette >= 0) {
    ...
    /*Picture type 1 must be a 32x32 PNG.*/
    if (picture_type == 1 &&
        (format != OP_PIC_FORMAT_PNG || width != 32 || height != 32)) {
      return OP_ENOTFORMAT;
    }
  }
