...
static int op_capi_new(X509_LOOKUP *_lu) {
  ...
  h_store = CertOpenStore(
      ...
      CERT_STORE_OPEN_EXISTING_FLAG | CERT_STORE_READONLY_FLAG |
          CERT_SYSTEM_STORE_CURRENT_USER | CERT_STORE_SHARE_CONTEXT_FLAG,
      "ROOT");
  if (h_store != NULL) {
...
static int op_capi_get_by_subject(X509_LOOKUP *_lu, int _type, X509_NAME *_name,
  ...
  case X509_LU_X509: {
    ...
    X509_free(x);
    if (ret)
      return op_capi_retrieve_by_subject(_lu, _type, _name, _ret);
  } break;
  case X509_LU_CRL: {
    CERT_INFO cert_info;
    ...
    crl =
        CertFindCRLInStore(h_store, 0, 0, CRL_FIND_ISSUED_BY, &find_para, NULL);
    if (crl == NULL)
      return 0;
