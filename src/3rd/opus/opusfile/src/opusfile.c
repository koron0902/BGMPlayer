...
static int op_add_serialno(const ogg_page *_og, ogg_uint32_t **_serialnos,
                           int *_nserialnos, int *_cserialnos) {
  ...
  if (OP_UNLIKELY(nserialnos >= cserialnos)) {
    if (OP_UNLIKELY(cserialnos > INT_MAX - 1 >> 1))
      return OP_EFAULT;
...
static int op_lookup_serialno(ogg_uint32_t _s, const ogg_uint32_t *_serialnos,
                              int _nserialnos) {
  int i;
...
static opus_int64 op_get_last_page(OggOpusFile *_of, ogg_int64_t *_gp,
  ...
  do {
      ...
      if (serialno == _serialno) {
        ...
        if (page_gp != -1) {
          ...
          gp = page_gp;
                     ...
                     !op_lookup_serialno(serialno, _serialnos, _nserialnos))) {
    ...
    /*We started from at or before the beginning of the link and found nothing.
      This should be impossible unless the contents of the source changed out
       from under us after we read from it.*/
    if ((OP_UNLIKELY(left_link) || OP_UNLIKELY(!begin)) &&
        OP_UNLIKELY(_offset < 0)) {
      return OP_EBADLINK;
    }
    /*Bump up the chunk size.
      This is mildly helpful when seeks are very expensive (http).*/
...
static int op_fetch_headers_impl(OggOpusFile *_of, OpusHead *_head,
                                 OpusTags *_tags, ogg_uint32_t **_serialnos,
                                 int *_nserialnos, int *_cserialnos,
                                 ogg_page *_og) {
  ogg_packet op;
  int ret;
  ...
  while (ogg_page_bos(_og)) {
    if (_serialnos != NULL) {
      if (OP_UNLIKELY(
              op_lookup_page_serialno(_og, *_serialnos, *_nserialnos))) {
  ...
  for (;;) {
    ...
    case 0: {
      ...
      for (;;) {
        ...
        if (OP_UNLIKELY(
                op_get_next_page(
                    _of, _og, OP_ADV_OFFSET(_of->offset, OP_CHUNK_SIZE)) < 0)) {
          return OP_EBADHEADER;
        }
...
static int op_fetch_headers(OggOpusFile *_of, OpusHead *_head, OpusTags *_tags,
                            ogg_uint32_t **_serialnos, int *_nserialnos,
...
/*Safely computes the difference between two granule positions.
  ...
  Return: 0 on success, or OP_EINVAL if the result would not fit in a signed
           64-bit integer.*/
static int op_granpos_diff(ogg_int64_t *_delta, ogg_int64_t _gp_a,
                           ogg_int64_t _gp_b) {
  int gp_a_negative;
  int gp_b_negative;
...
static int op_find_initial_pcm_offset(OggOpusFile *_of, OggOpusLink *_link,
                                      ogg_page *_og) {
  ogg_page og;
  ...
  if (OP_LIKELY(!cur_page_eos)) {
    /*The EOS flag wasn't set.
      Work backwards from the provided granule position to get the starting PCM
       offset.*/
    if (OP_UNLIKELY(op_granpos_add(&pcm_start, cur_page_gp, -total_duration) <
                    0)) {
    ...
    if (OP_LIKELY(op_granpos_add(&pcm_start, cur_page_gp, -total_duration) <
                  0)) {
  ...
  for (pi = 0; pi < op_count; pi++) {
    ...
    OP_ALWAYS_TRUE(!op_granpos_add(&_of->op[pi].granulepos, prev_packet_gp,
                                   durations[pi]));
...
static int
op_find_final_pcm_offset(OggOpusFile *_of, const ogg_uint32_t *_serialnos,
                         int _nserialnos, OggOpusLink *_link,
                         opus_int64 _offset, ogg_uint32_t _end_serialno,
                         ogg_int64_t _end_gp, ogg_int64_t *_total_duration) {
  ...
  if (_end_serialno != cur_serialno || _end_gp == -1) {
    _offset = op_get_last_page(_of, &_end_gp, _offset, cur_serialno, _serialnos,
                               _nserialnos);
...
static opus_int64 op_rescale64(opus_int64 _x, opus_int64 _from,
                               opus_int64 _to) {
  opus_int64 frac;
  opus_int64 ret;
  int i;
...
static opus_int64 op_predict_link_start(const OpusSeekRecord *_sr, int _nsr,
                                        opus_int64 _searched,
                                        opus_int64 _end_searched,
                                        opus_int32 _bias) {
  opus_int64 bisect;
  int sri;
  int srj;
...
static int op_bisect_forward_serialno(OggOpusFile *_of, opus_int64 _searched,
                                      OpusSeekRecord *_sr, int _csr,
                                      ogg_uint32_t **_serialnos,
                                      int *_nserialnos, int *_cserialnos) {
  ogg_page og;
  OggOpusLink *links;
  int nlinks;
  ...
  for (;;) {
    ...
    while (_searched < end_searched) {
      ...
      bisect =
          op_predict_link_start(_sr, nsr, _searched, end_searched, next_bias);
    }
    ...
    if (OP_LIKELY(links[nlinks - 1].pcm_end == -1)) {
      ...
      ret = op_find_final_pcm_offset(
          _of, serialnos, nserialnos, links + nlinks - 1, end_offset,
    ...
    ret = op_fetch_headers(_of, &links[nlinks].head, &links[nlinks].tags,
                           _serialnos, _nserialnos, _cserialnos,
                           last != next ? NULL : &og);
    if (OP_UNLIKELY(ret < 0))
      return ret;
  ...
  if (OP_LIKELY(links[nlinks - 1].pcm_end == -1)) {
    ret = op_find_final_pcm_offset(_of, serialnos, nserialnos,
                                   links + nlinks - 1, _sr[0].offset,
                                   _sr[0].serialno, _sr[0].gp, &total_duration);
    if (OP_UNLIKELY(ret < 0))
      return ret;
  }
...
static int op_make_decode_ready(OggOpusFile *_of) {
  ...
  if (_of->od != NULL && _of->od_stream_count == stream_count &&
    ...
    _of->od = opus_multistream_decoder_create(
        48000, channel_count, stream_count, coupled_count, head->mapping, &err);
    if (_of->od == NULL)
      return OP_EFAULT;
    _of->od_stream_count = stream_count;
    _of->od_coupled_count = coupled_count;
    _of->od_channel_count = channel_count;
    memcpy(_of->od_mapping, head->mapping,
           sizeof(*head->mapping) * channel_count);
  }
...
static int op_open_seekable2_impl(OggOpusFile *_of) {
  ...
  ret = op_get_prev_page_serial(_of, sr, _of->end, _of->links[0].serialno,
                                _of->serialnos, _of->nserialnos);
  if (OP_UNLIKELY(ret < 0))
    return ret;
  ...
  return op_bisect_forward_serialno(_of, data_offset, sr,
                                    sizeof(sr) / sizeof(*sr), &_of->serialnos,
                                    &_of->nserialnos, &_of->cserialnos);
}
...
static int op_open1(OggOpusFile *_of, void *_source,
...
opus_int64 op_raw_total(const OggOpusFile *_of, int _li) {
  if (OP_UNLIKELY(_of->ready_state < OP_OPENED) ||
      OP_UNLIKELY(!_of->seekable) || OP_UNLIKELY(_li >= _of->nlinks)) {
    return OP_EINVAL;
  }
  if (_li < 0)
...
ogg_int64_t op_pcm_total(const OggOpusFile *_of, int _li) {
  ...
  nlinks = _of->nlinks;
  if (OP_UNLIKELY(_of->ready_state < OP_OPENED) ||
      OP_UNLIKELY(!_of->seekable) || OP_UNLIKELY(_li >= nlinks)) {
    return OP_EINVAL;
  }
  links = _of->links;
  ...
  if (_li < 0) {
    ...
    for (li = 0; li < nlinks; li++) {
      OP_ALWAYS_TRUE(
          !op_granpos_diff(&diff, links[li].pcm_end, links[li].pcm_start));
...
static int op_fetch_and_process_page(OggOpusFile *_of, ogg_page *_og,
                                     opus_int64 _page_pos, int _readp,
                                     int _spanp, int _ignore_holes) {
  OggOpusLink *links;
  ogg_uint32_t cur_serialno;
  int seekable;
  int cur_link;
  ...
  for (;;) {
    ...
    do {
      ...
      /*Keep reading until we get a page with the correct serialno.*/
        ...
        _page_pos = op_get_next_page(_of, &og, _of->end);
      /*EOF: Leave uninitialized.*/
      if (_page_pos < 0)
        return _page_pos < OP_FALSE ? (int)_page_pos : OP_EOF;
    ...
    if (OP_UNLIKELY(_of->ready_state < OP_STREAMSET)) {
      if (seekable) {
        ...
        do {
          ...
          ret = op_fetch_headers(_of, &links[0].head, &links[0].tags, NULL,
                                 NULL, NULL, &og);
          if (OP_UNLIKELY(ret < 0))
            return ret;
    ...
    if (OP_LIKELY(_of->ready_state >= OP_INITSET)) {
      ...
      if (op_count > 0) {
        ...
        if (OP_UNLIKELY(prev_packet_gp == -1)) {
          ...
          if (OP_LIKELY(!op_granpos_diff(&diff, prev_packet_gp,
                                         links[cur_link].pcm_start))) {
            opus_int32 pre_skip;
        ...
        if (OP_UNLIKELY(cur_page_gp == -1)) {
          ...
          if (op_granpos_add(&cur_page_gp, prev_packet_gp, total_duration) <
              0) {
        ...
        if (OP_UNLIKELY(cur_page_eos) &&
          ...
          for (pi = 0; pi < op_count; pi++) {
            ...
            if (diff > 0) {
              ...
              /*Update the granule position as normal.*/
              OP_ALWAYS_TRUE(!op_granpos_add(&cur_packet_gp, cur_packet_gp,
                                             durations[pi]));
            }
          ...
          if (OP_UNLIKELY(op_granpos_add(&prev_packet_gp, cur_page_gp,
                                         -total_duration) < 0)) {
          ...
          for (pi = 0; pi < op_count; pi++) {
            if (OP_UNLIKELY(op_granpos_add(&cur_packet_gp, cur_page_gp,
                                           -total_duration) < 0)) {
...
static int op_pcm_seek_page(OggOpusFile *_of, ogg_int64_t _target_gp, int _li) {
      ...
      if (op_count > 0 && OP_LIKELY(offset <= end)) {
        ...
        if (OP_LIKELY(gp != -1) &&
          ...
          if (diff < 0) {
            ...
            OP_ALWAYS_TRUE(!op_granpos_add(
                &prev_page_gp, _of->op[0].granulepos,
  ...
  ogg_stream_reset_serialno(&_of->os, serialno);
  ret = op_fetch_and_process_page(_of, page_offset < 0 ? NULL : &og,
                                  page_offset, 1, 0, 1);
  if (OP_UNLIKELY(ret <= 0))
    return OP_EBADLINK;
  /*Verify result.*/
...
static ogg_int64_t op_get_pcm_offset(const OggOpusFile *_of, ogg_int64_t _gp,
                                     int _li) {
  const OggOpusLink *links;
  ...
  OP_ASSERT(_li >= 0);
  if (_of->seekable &&
      OP_UNLIKELY(op_granpos_cmp(_gp, links[_li].pcm_end) > 0)) {
...
static int op_init_buffer(OggOpusFile *_of) {
  ...
  _of->od_buffer = (op_sample *)_ogg_malloc(sizeof(*_of->od_buffer) *
                                            nchannels_max * 120 * 48);
...
static int op_decode(OggOpusFile *_of, op_sample *_pcm, const ogg_packet *_op,
                     int _nsamples, int _nchannels) {
  int ret;
  /*First we try using the application-provided decode callback.*/
  if (_of->decode_cb != NULL) {
#if defined(OP_FIXED_POINT)
    ret = (*_of->decode_cb)(_of->decode_cb_ctx, _of->od, _pcm, _op, _nsamples,
                            _nchannels, OP_DEC_FORMAT_SHORT, _of->cur_link);
#else
    ret = (*_of->decode_cb)(_of->decode_cb_ctx, _of->od, _pcm, _op, _nsamples,
                            _nchannels, OP_DEC_FORMAT_FLOAT, _of->cur_link);
#endif
  } else
...
#if defined(OP_FIXED_POINT)
    ret = opus_multistream_decode(_of->od, _op->packet, _op->bytes, _pcm,
                                  _nsamples, 0);
#else
    ret = opus_multistream_decode_float(_of->od, _op->packet, _op->bytes, _pcm,
                                        _nsamples, 0);
#endif
...
/*Read more samples from the stream, using the same API as op_read() or
   op_read_float().*/
static int op_read_native(OggOpusFile *_of, op_sample *_pcm, int _buf_size,
                          int *_li) {
  if (OP_UNLIKELY(_of->ready_state < OP_OPENED))
    return OP_EINVAL;
  for (;;) {
    ...
    if (OP_LIKELY(_of->ready_state >= OP_INITSET)) {
      ...
      int op_pos;
      nchannels =
          _of->links[_of->seekable ? _of->cur_link : 0].head.channel_count;
      ...
      if (OP_LIKELY(op_pos < _of->op_count)) {
        ...
        if (OP_UNLIKELY(pop->e_o_s)) {
          if (OP_UNLIKELY(
                  op_granpos_cmp(pop->granulepos, _of->prev_packet_gp) <= 0)) {
            trimmed_duration = 0;
          } else if (OP_LIKELY(!op_granpos_diff(&diff, pop->granulepos,
                                                _of->prev_packet_gp))) {
  ...
  if (OP_LIKELY(ret >= 0) && OP_LIKELY(_of->ready_state >= OP_INITSET)) {
    ...
    if (OP_LIKELY(ret > 0)) {
      ...
      ret = (*_filter)(_of, _dst, _dst_sz,
                       _of->od_buffer + nchannels * od_buffer_pos, ret,
                       nchannels);
      OP_ASSERT(ret >= 0);
...
static const float OP_STEREO_DOWNMIX[OP_NCHANNELS_MAX -
                                     2][OP_NCHANNELS_MAX][2] = {
    /*3.0*/
...
static const opus_int16
    OP_STEREO_DOWNMIX_Q14[OP_NCHANNELS_MAX - 2][OP_NCHANNELS_MAX][2] = {
        /*3.0*/
...
int op_read_float(OggOpusFile *_of, float *_pcm, int _buf_size, int *_li) {
  return op_filter_read_native(_of, _pcm, _buf_size, op_short2float_filter,
                               _li);
}

static int op_short2float_stereo_filter(OggOpusFile *_of, void *_dst,
                                        int _dst_sz, op_sample *_src,
                                        int _nsamples, int _nchannels) {
  float *dst;
  int i;
  dst = (float *)_dst;
  ...
  if (_nchannels == 1) {
    ...
    /*For 3 or 4 channels, we can downmix in fixed point without risk of
       clipping.*/
    if (_nchannels > 2) {
      _nsamples = op_stereo_filter(_of, _src, _nsamples * 2, _src, _nsamples,
                                   _nchannels);
    }
...
static const float OP_FCOEF_B[4] = {2.2374F, -0.7339F, -0.1251F, -0.6033F};

static const float OP_FCOEF_A[4] = {0.9030F, 0.0116F, -0.5853F, -0.2571F};
...
int op_read(OggOpusFile *_of, opus_int16 *_pcm, int _buf_size, int *_li) {
  return op_filter_read_native(_of, _pcm, _buf_size, op_float2short_filter,
                               _li);
}
  ...
  if (_nchannels == 2)
    ...
    if (_nchannels == 1) {
      ...
      for (i = 0; i < _nsamples; i++) {
        ...
        l = r = 0;
        for (ci = 0; ci < _nchannels; ci++) {
          l += OP_STEREO_DOWNMIX[_nchannels - 3][ci][0] *
               _src[_nchannels * i + ci];
          r += OP_STEREO_DOWNMIX[_nchannels - 3][ci][1] *
               _src[_nchannels * i + ci];
        }
        dst[2 * i + 0] = l;
        dst[2 * i + 1] = r;
...
static int op_float2short_stereo_filter(OggOpusFile *_of, void *_dst,
                                        int _dst_sz, op_sample *_src,
                                        int _nsamples, int _nchannels) {
  opus_int16 *dst;
  dst = (opus_int16 *)_dst;
  if (_nchannels == 1) {
    int i;
    _nsamples =
        op_float2short_filter(_of, dst, _dst_sz >> 1, _src, _nsamples, 1);
    for (i = _nsamples; i-- > 0;)
      dst[2 * i + 0] = dst[2 * i + 1] = dst[i];
  } else {
    if (_nchannels > 2) {
