...
typedef struct {
  ...
  struct {
    int last_dbits[2], last_delta_sign[2], bit_limit;
  } w2;

  struct {
    int ave_dbits[2], bit_limit;
  } w3;
...
#define SAVE(destin, item)                                                     \
  {                                                                            \
    memcpy(destin, &item, sizeof(item));                                       \
    destin = (char *)destin + sizeof(item);                                    \
  }
#define RESTORE(item, source)                                                  \
  {                                                                            \
    memcpy(&item, source, sizeof(item));                                       \
    source = (char *)source + sizeof(item);                                    \
  }
