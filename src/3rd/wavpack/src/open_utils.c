...
static int seek_eof_information(WavpackContext *wpc, int64_t *final_index,
                                int get_wrapper);

WavpackContext *WavpackOpenFileInputEx64(WavpackStreamReader64 *reader,
                                         void *wv_id, void *wvc_id, char *error,
  ...
  wpc->max_streams =
      OLD_MAX_STREAMS; // use this until overwritten with actual number
  wpc->open_flags = flags;

  wpc->filelen = wpc->reader->get_length(wpc->wv_in);

#ifndef NO_TAGS
  if ((flags & (OPEN_TAGS | OPEN_EDIT_TAGS)) &&
      wpc->reader->can_seek(wpc->wv_in)) {
    load_tag(wpc);
    wpc->reader->set_pos_abs(wpc->wv_in, 0);

    if ((flags & OPEN_EDIT_TAGS) && !editable_tag(&wpc->m_tag)) {
      if (error)
        strcpy(error, "can't edit tags located at the beginning of files!");
...
#else
    if (error)
      strcpy(error, "this legacy WavPack file is deprecated, use version "
                    "4.80.0 to transcode");
    return WavpackCloseFile(wpc);
#endif
  }

  wpc->streams = (WavpackStream **)(malloc((wpc->num_streams = 1) *
                                           sizeof(wpc->streams[0])));
  if (!wpc->streams) {
    if (error)
      strcpy(error, "can't allocate memory");
    return WavpackCloseFile(wpc);
  ...
  while (!wps->wphdr.block_samples) {
    ...
    if (wpc->reader->read_bytes(wpc->wv_in, wps->blockbuff + 32,
                                wps->wphdr.ckSize - 24) !=
        wps->wphdr.ckSize - 24) {
      if (error)
        strcpy(error, "can't read all of WavPack file!");
    ...
    if (!WavpackVerifySingleBlock(wps->blockbuff,
    ...
    if (wps->wphdr.block_samples) {
      if (flags & OPEN_STREAMING)
        ...
        if (GET_BLOCK_INDEX(wps->wphdr) ||
            GET_TOTAL_SAMPLES(wps->wphdr) == -1) {
    ...
    } else if (wpc->total_samples == -1 && !GET_BLOCK_INDEX(wps->wphdr) &&
               GET_TOTAL_SAMPLES(wps->wphdr))
      wpc->total_samples = GET_TOTAL_SAMPLES(wps->wphdr);

    if (wpc->wvc_in && wps->wphdr.block_samples &&
        (wps->wphdr.flags & HYBRID_FLAG)) {
      unsigned char ch;
  ...
  if (wps->wphdr.flags & DSD_FLAG) {
    ...
    if (flags & OPEN_DSD_NATIVE) {
      ...
      wpc->decimation_context =
          decimate_dsd_init(wpc->reduced_channels ? wpc->reduced_channels
                                                  : wpc->config.num_channels);

      wpc->config.bytes_per_sample = 3;
      wpc->config.bits_per_sample = 24;
    } else {
      if (error)
        strcpy(error, "not configured to handle DSD WavPack files!");
      return WavpackCloseFile(wpc);
    }
#else
    if (error)
  ...
  if (!wpc->config.sample_rate) {
    if (!wps->wphdr.block_samples ||
        (wps->wphdr.flags & SRATE_MASK) == SRATE_MASK)
      wpc->config.sample_rate = 44100;
    else
      wpc->config.sample_rate =
          sample_rates[(wps->wphdr.flags & SRATE_MASK) >> SRATE_LSB];
  }
...
static int read_metadata_buff(WavpackMetadata *wpmd, unsigned char *blockbuff,
...
int unpack_init(WavpackContext *wpc) {
  ...
  if (!(wps->wphdr.flags & MONO_FLAG) && wpc->config.num_channels &&
  ...
  if ((wps->wphdr.flags & UNKNOWN_FLAGS) ||
      (wps->wphdr.flags & MONO_DATA) == MONO_DATA) {
    wps->mute_error = TRUE;
    return FALSE;
  }
  ...
  if (wps->wphdr.block_samples &&
      ((wps->wphdr.flags & DSD_FLAG) ? !wps->dsd.ready
                                     : !bs_is_open(&wps->wvbits))) {
  ...
  if (wps->wphdr.block_samples && !bs_is_open(&wps->wvxbits)) {
    ...
    if ((wps->wphdr.flags & FLOAT_DATA) &&
        wps->float_flags & (FLOAT_EXCEPTIONS | FLOAT_ZEROS_SENT |
...
static int init_wv_bitstream(WavpackStream *wps, WavpackMetadata *wpmd) {
  ...
  bs_open_read(&wps->wvbits, wpmd->data,
...
static int init_wvc_bitstream(WavpackStream *wps, WavpackMetadata *wpmd) {
  ...
  bs_open_read(&wps->wvcbits, wpmd->data,
               (unsigned char *)wpmd->data + wpmd->byte_length);
  return TRUE;
}
...
static int read_channel_info(WavpackContext *wpc, WavpackMetadata *wpmd) {
  ...
  if (!wpc->config.num_channels) {
    ...
    // if bytecnt is 6 or 7 we are using new configuration with "unlimited"
    // streams
...
static int read_new_config_info(WavpackContext *wpc, WavpackMetadata *wpmd) {
  ...
  if (bytecnt >= 2) {
    ...
    if (bytecnt) {
      ...
      // another byte means we have a channel count for the layout and maybe a
      // reordering
      ...
      if (bytecnt) {
        ...
        if (bytecnt) {
          ...
          // note that redundant reordering info is not stored, so we fill in
          // the rest
          ...
          if (wpc->channel_reordering) {
            for (i = 0; i < nchans; ++i)
              if (bytecnt) {
                ...
                if (wpc->channel_reordering[i] >=
                    nchans) // make sure index is in range
...
static int read_wrapper_data(WavpackContext *wpc, WavpackMetadata *wpmd) {
  if ((wpc->open_flags & OPEN_WRAPPER) &&
      wpc->wrapper_bytes < MAX_WRAPPER_BYTES && wpmd->byte_length) {
    wpc->wrapper_data = (unsigned char *)realloc(
        wpc->wrapper_data, wpc->wrapper_bytes + wpmd->byte_length);
    if (!wpc->wrapper_data)
      return FALSE;
    memcpy(wpc->wrapper_data + wpc->wrapper_bytes, wpmd->data,
           wpmd->byte_length);
...
// Normally the trailing wrapper will not be available when a WavPack file is
// first opened for reading because it is stored in the final block of the file.
// This function forces a seek to the end of the file to pick up any trailing
// wrapper stored there (then use WavPackGetWrapper**() to obtain). This can
// obviously only be used for seekable files (not pipes) and is not available
// for pre-4.0 WavPack files.

void WavpackSeekTrailingWrapper(WavpackContext *wpc) {
  if ((wpc->open_flags & OPEN_WRAPPER) && wpc->reader->can_seek(wpc->wv_in) &&
      !wpc->stream3)
...
uint32_t read_next_header(WavpackStreamReader64 *reader, void *id,
                          WavpackHeader *wphdr) {
  ...
  while (1) {
    ...
    if (reader->read_bytes(id, buffer + bleft, sizeof(*wphdr) - bleft) !=
        sizeof(*wphdr) - bleft)
      return -1;
    ...
    if (*sp++ == 'w' && *sp == 'v' && *++sp == 'p' && *++sp == 'k' &&
        !(*++sp & 1) && sp[2] < 16 && !sp[3] && (sp[2] || sp[1] || *sp >= 24) &&
        sp[5] == 4 && sp[4] >= (MIN_STREAM_VERS & 0xff) &&
...
int read_wvc_block(WavpackContext *wpc) {
  ...
  while (1) {
    ...
    if (!compare_result) {
      ...
      if (wpc->reader->read_bytes(wpc->wvc_in, wps->block2buff + 32,
                                  wphdr.ckSize - 24) != wphdr.ckSize - 24) {
        free(wps->block2buff);
        wps->block2buff = NULL;
        wps->wvc_skip = TRUE;
        wpc->crc_errors++;
      ...
      if (!WavpackVerifySingleBlock(wps->block2buff,
                                    !(wpc->open_flags & OPEN_NO_CHECKSUM))) {
        free(wps->block2buff);
        wps->block2buff = NULL;
        wps->wvc_skip = TRUE;
        wpc->crc_errors++;
...
static int seek_eof_information(WavpackContext *wpc, int64_t *final_index,
  ...
  // start 1MB from the end-of-file, or from the start if the file is not that
  // big
  ...
  // Note that we go backward (without parsing inside blocks) until we find a
  // block with audio (careful to not get stuck in a loop). Only then do we go
  ...
  while (1) {
    ...
    // if we just got to the same place as last time, we're stuck and need to
    // give up
    ...
    // We enter here if we just read 1 MB without seeing any WavPack block
    // headers. Since WavPack blocks are < 1 MB, that means we're in a big APE
    // tag, or we got to the end-of-file.

    if (bcount == (uint32_t)-1) {

      // if we have not seen any blocks at all yet, back up almost 2 MB (or to
      // the beginning of the file) and try again
      ...
      // if we have seen WavPack blocks, then this means we've done all we can
      // do here
    ...
    // If the block has audio samples, calculate a final index, although this is
    // not final since this may not be the last block with audio. On the other
    // hand, if this block does not have audio, and we haven't seen one with
    // audio, we have to go back some more.
    ...
    // entire block looking for MD5 metadata or (conditionally) trailing
    // wrappers
    ...
    while (bcount >= 2) {
      ...
      if (get_wrapper &&
          (meta_id == ID_RIFF_TRAILER ||
           (alt_types && meta_id == ID_ALT_TRAILER)) &&
          meta_bc) {
        wpc->wrapper_data = (unsigned char *)realloc(
            wpc->wrapper_data, wpc->wrapper_bytes + meta_bc);
        ...
        if (reader->read_bytes(id, wpc->wrapper_data + wpc->wrapper_bytes,
                               meta_bc) == meta_bc)
      ...
      } else if (meta_id == ID_MD5_CHECKSUM ||
                 (alt_types && meta_id == ID_ALT_MD5_CHECKSUM)) {
...
// Quickly verify the referenced block. It is assumed that the WavPack header
// has been converted to native endian format. If a block checksum is performed,
// that is done in little-endian (file) format. It is also assumed that the
// caller has made sure that the block length indicated in the header is correct
// (we won't overflow the buffer). If a checksum is present, then it is checked,
// otherwise we just check that all the metadata blocks are formatted correctly
// (without looking at their contents). Returns FALSE for bad block.
...
int WavpackVerifySingleBlock(unsigned char *buffer, int verify_checksum) {
  ...
  if (strncmp(wphdr->ckID, "wvpk", 4) ||
      wphdr->ckSize + 8 < sizeof(WavpackHeader))
    return FALSE;
  ...
  return (bcount == 0) && (!verify_checksum || !(wphdr->flags & HAS_CHECKSUM) ||
                           checksum_passed);
}
