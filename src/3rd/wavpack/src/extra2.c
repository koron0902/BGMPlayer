...
#define PACK_DECORR_STEREO_PASS_CONT_AVAILABLE                                 \
...
void PACK_DECORR_STEREO_PASS_CONT(struct decorr_pass *dpp, int32_t *in_buffer,
                                  int32_t *out_buffer, int32_t sample_count);
void PACK_DECORR_STEREO_PASS_CONT_REV(struct decorr_pass *dpp,
                                      int32_t *in_buffer, int32_t *out_buffer,
...
static void decorr_stereo_pass(int32_t *in_samples, int32_t *out_samples,
                               int32_t num_samples, struct decorr_pass *dpp,
                               int dir) {
  ...
  if (num_samples > 16 && PACK_DECORR_STEREO_PASS_CONT_AVAILABLE) {
    int32_t pre_samples =
        (dpp->term < 0 || dpp->term > MAX_TERM) ? 2 : dpp->term;
  ...
  case 2:
    while (num_samples--) {
      ...
      out_samples[0] = tmp = (dpp->samples_A[1] = in_samples[0]) -
                             apply_weight(dpp->weight_A, sam);
      update_weight(dpp->weight_A, dpp->delta, sam, tmp);
      dpp->sum_A += dpp->weight_A;

      sam = dpp->samples_B[0];
      dpp->samples_B[0] = dpp->samples_B[1];
      out_samples[1] = tmp = (dpp->samples_B[1] = in_samples[1]) -
                             apply_weight(dpp->weight_B, sam);
  ...
  case 17:
    while (num_samples--) {
      ...
      out_samples[0] = tmp = (dpp->samples_A[0] = in_samples[0]) -
                             apply_weight(dpp->weight_A, sam);
      update_weight(dpp->weight_A, dpp->delta, sam, tmp);
      dpp->sum_A += dpp->weight_A;
      ...
      out_samples[1] = tmp = (dpp->samples_B[0] = in_samples[1]) -
                             apply_weight(dpp->weight_B, sam);
      update_weight(dpp->weight_B, dpp->delta, sam, tmp);
      dpp->sum_B += dpp->weight_B;
  ...
  default: {
    ...
    while (num_samples--) {
      ...
      sam = dpp->samples_A[m];
      out_samples[0] = tmp = (dpp->samples_A[k] = in_samples[0]) -
                             apply_weight(dpp->weight_A, sam);
      update_weight(dpp->weight_A, dpp->delta, sam, tmp);
      dpp->sum_A += dpp->weight_A;

      sam = dpp->samples_B[m];
      out_samples[1] = tmp = (dpp->samples_B[k] = in_samples[1]) -
                             apply_weight(dpp->weight_B, sam);
      update_weight(dpp->weight_B, dpp->delta, sam, tmp);
      dpp->sum_B += dpp->weight_B;
  ...
  case -1:
    while (num_samples--) {
      ...
      out_samples[0] = tmp =
          (sam_B = in_samples[0]) - apply_weight(dpp->weight_A, sam_A);
      update_weight_clip(dpp->weight_A, dpp->delta, sam_A, tmp);
      dpp->sum_A += dpp->weight_A;

      out_samples[1] = tmp = (dpp->samples_A[0] = in_samples[1]) -
                             apply_weight(dpp->weight_B, sam_B);
  ...
  case -2:
    while (num_samples--) {
      ...
      out_samples[1] = tmp =
          (sam_A = in_samples[1]) - apply_weight(dpp->weight_B, sam_B);
      update_weight_clip(dpp->weight_B, dpp->delta, sam_B, tmp);
      dpp->sum_B += dpp->weight_B;

      out_samples[0] = tmp = (dpp->samples_B[0] = in_samples[0]) -
                             apply_weight(dpp->weight_A, sam_A);
      update_weight_clip(dpp->weight_A, dpp->delta, sam_A, tmp);
      dpp->sum_A += dpp->weight_A;
  ...
  if (cont_samples) {
    if (dir < 0)
      PACK_DECORR_STEREO_PASS_CONT_REV(dpp, in_samples, out_samples,
                                       cont_samples);
    else
...
static void decorr_stereo_buffer(WavpackExtraInfo *info, int32_t *samples,
                                 int32_t *outsamples, int32_t num_samples,
                                 int tindex) {
  ...
  dp.term = term;
  dp.delta = pre_delta;
  decorr_stereo_pass(samples, outsamples,
                     num_samples > 2048 ? 2048 : num_samples, &dp, -1);
  dp.delta = delta;
...
static void recurse_stereo(WavpackContext *wpc, WavpackExtraInfo *info,
                           int depth, int delta, uint32_t input_bits) {
  WavpackStream *wps = wpc->streams[wpc->current_stream];
  int term,
      branches = ((wpc->config.extra_flags & EXTRA_BRANCHES) >> 6) - depth;
  ...
  for (term = -3; term <= 18; ++term) {
    ...
    decorr_stereo_buffer(info, samples, outsamples, wps->wphdr.block_samples,
                         depth);
    bits =
        LOG2BUFFER(outsamples, wps->wphdr.block_samples * 2, info->log_limit);
    ...
    if (bits < info->best_bits) {
      ...
      memcpy(info->sampleptrs[info->nterms + 1], info->sampleptrs[depth + 1],
             wps->wphdr.block_samples * 8);
    }
  ...
  while (depth + 1 < info->nterms && branches--) {
    ...
    //      if (log2buffer (outsamples, wps->wphdr.block_samples * 2, 0) !=
    //      local_best_bits)
...
static void delta_stereo(WavpackContext *wpc, WavpackExtraInfo *info) {
  ...
  for (d = delta - 1; d >= 0; --d) {
    ...
    for (i = 0; i < info->nterms && wps->decorr_passes[i].term; ++i) {
      ...
      info->dps[i].delta = d;
      decorr_stereo_buffer(info, info->sampleptrs[i], info->sampleptrs[i + 1],
                           wps->wphdr.block_samples, i);
    }

    bits = LOG2BUFFER(info->sampleptrs[i], wps->wphdr.block_samples * 2,
                      info->log_limit);
    ...
    if (bits < info->best_bits) {
      ...
      memcpy(info->sampleptrs[info->nterms + 1], info->sampleptrs[i],
             wps->wphdr.block_samples * 8);
    } else
      break;
  }
  ...
  while (reversed) {
    ...
    for (ri = 0; ri < info->nterms && wps->decorr_passes[ri].term; ++ri) {
      ...
      if (wps->decorr_passes[ri].term == wps->decorr_passes[ri + 1].term) {
        decorr_stereo_buffer(info, info->sampleptrs[ri],
                             info->sampleptrs[ri + 1], wps->wphdr.block_samples,
                             ri);
        continue;
      }
...
static void analyze_stereo(WavpackContext *wpc, int32_t *samples,
                           int do_samples) {
  WavpackStream *wps = wpc->streams[wpc->current_stream];
  ...
  for (i = 0; i < info.nterms && info.dps[i].term; ++i)
    decorr_stereo_pass(info.sampleptrs[i], info.sampleptrs[i + 1],
                       wps->wphdr.block_samples, info.dps + i, 1);

  info.best_bits = LOG2BUFFER(info.sampleptrs[info.nterms],
                              wps->wphdr.block_samples * 2, 0) *
                   1;
  ...
  if (wpc->config.extra_flags & EXTRA_BRANCHES)
    recurse_stereo(
        wpc, &info, 0, (int)floor(wps->delta_decay + 0.5),
  ...
  if (wpc->config.extra_flags & EXTRA_TRY_DELTAS) {
    ...
    if ((wpc->config.extra_flags & EXTRA_ADJUST_DELTAS) &&
        wps->decorr_passes[0].term)
      wps->delta_decay =
          (float)((wps->delta_decay * 2.0 + wps->decorr_passes[0].delta) / 3.0);
    else
      wps->delta_decay = 2.0;
  }
  ...
  if (do_samples)
    memcpy(samples, info.sampleptrs[info.nterms + 1],
           wps->wphdr.block_samples * 8);
...
static void stereo_add_noise(WavpackStream *wps, int32_t *lptr, int32_t *rptr) {
  ...
  if (wps->wphdr.flags & HYBRID_SHAPE) {
    ...
    if (!shaping_array) {
      wps->dc.shaping_acc[0] -=
          wps->dc.shaping_delta[0] * wps->wphdr.block_samples;
      wps->dc.shaping_acc[1] -=
          wps->dc.shaping_delta[1] * wps->wphdr.block_samples;
    }
  } else
    while (cnt--) {
...
void execute_stereo(WavpackContext *wpc, int32_t *samples, int no_history,
                    int do_samples) {
  ...
  if (wps->num_passes > 1 && (wps->wphdr.flags & HYBRID_FLAG)) {
    ...
    decorr_stereo_pass(samples, temp_buffer[0],
                       num_samples > 2048 ? 2048 : num_samples,
                       &temp_decorr_pass, -1);

    reverse_decorr(&temp_decorr_pass);
    decorr_stereo_pass(samples, temp_buffer[0], num_samples, &temp_decorr_pass,
                       1);
    CLEAR(temp_decorr_pass);
    temp_decorr_pass.delta = 2;
    ...
    decorr_stereo_pass(temp_buffer[0], temp_buffer[1], num_samples,
                       &temp_decorr_pass, 1);
  ...
  for (pi = 0; pi < wps->num_passes;) {
    ...
    if (!pi)
      ...
      if (c == wps->best_decorr) {
        wps->mask_decorr =
            wps->mask_decorr
                ? ((wps->mask_decorr << 1) & (wps->num_decorrs - 1))
                : 1;
        continue;
      }
    }
    ...
    while (1) {
      ...
      for (j = 0; j < nterms; ++j) {
        ...
        memcpy(save_decorr_passes + j, &temp_decorr_pass,
               sizeof(struct decorr_pass));
        decorr_stereo_pass(temp_buffer[j & 1], temp_buffer[~j & 1], num_samples,
                           &temp_decorr_pass, 1);
      }
    ...
    if (size < best_size) {
      ...
      memcpy(wps->decorr_passes, save_decorr_passes,
             sizeof(struct decorr_pass) * MAX_NTERMS);
    ...
    if (pi++)
      wps->mask_decorr =
          wps->mask_decorr ? ((wps->mask_decorr << 1) & (wps->num_decorrs - 1))
                           : 1;
  }

  if (force_js ||
      (wps->decorr_specs[wps->best_decorr].joint_stereo && !force_ts))
  ...
  if (wpc->config.xmode > 3 || no_history ||
  ...
  if (1) {
    ...
    sprintf(string,
            "%s: terms =", (wps->wphdr.flags & JOINT_STEREO) ? "JS" : "TS");
