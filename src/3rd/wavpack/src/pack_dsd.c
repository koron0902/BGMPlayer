...
static int encode_buffer_high(WavpackStream *wps, int32_t *buffer,
                              int num_samples, unsigned char *destination);
static int encode_buffer_fast(WavpackStream *wps, int32_t *buffer,
                              int num_samples, unsigned char *destination);
...
int pack_dsd_block(WavpackContext *wpc, int32_t *buffer) {
  ...
  dsd_encoding =
      wps->blockbuff + ((WavpackHeader *)wps->blockbuff)->ckSize + 12;
...
#define MAX_PROBABILITY                                                        \
  0xa0 // set to 0xff to disable RLE encoding for probabilities table

#if (MAX_PROBABILITY < 0xff)

static int rle_encode(unsigned char *src, int bcount,
  ...
  while (bcount--) {
    if (*src) {
      while (zcount) {
        *dp++ =
            MAX_PROBABILITY + (zcount > max_rle_zeros ? max_rle_zeros : zcount);
...
static void calculate_probabilities(int hist[256], unsigned char probs[256],
                                    unsigned short prob_sums[256]) {
...
#if 0 // this code reduces probability values when they are completely redundant
      // (i.e., common divisor), but this doesn't really happen often enough to
      // make it worthwhile
...
static int encode_buffer_fast(WavpackStream *wps, int32_t *buffer,
  ...
  for (p0 = 0; p0 < history_bins; p0++) {
    calculate_probabilities(histogram[p0], probabilities[p0],
                            summed_probabilities[p0]);
    total_summed_probabilities += summed_probabilities[p0][255];
  }

  ((WavpackHeader *)wps->blockbuff)->crc = crc;

  // This code detects the case where the required value lookup tables grow
  // silly big and cuts them back down. This would normally only happen with
  // large blocks or poorly compressible data. The target is to guarantee that
  // the total memory required for all three decode tables will be 2K bytes per
  // history bin.
  ...
  while (total_summed_probabilities > history_bins * 1280) {
    ...
    for (p1 = 0; p1 < 256; ++p1)
      summed_probabilities[p0][p1] = sum_values += probabilities[p0][p1] =
          (probabilities[p0][p1] + 1) >> 1;

    total_summed_probabilities += summed_probabilities[p0][255];
    // fprintf (stderr, "processed bin 0x%02x, bin: %d --> %d, new sum = %d\n",
    //     p0, max_sum, summed_probabilities [p0] [255],
    //     total_summed_probabilities);
  }
...
#if (MAX_PROBABILITY < 0xff)
  dp += rle_encode((unsigned char *)probabilities,
                   sizeof(*probabilities) * history_bins, dp);
#else
  ...
  *dp++ = 3;
  ...
  if (!wps->sample_index) {
    ...
    for (channel = 0; channel < 2; ++channel) {
      ...
      sp->filter1 = sp->filter2 = sp->filter3 = sp->filter4 = sp->filter5 =
          VALUE_ONE / 2;
