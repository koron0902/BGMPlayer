...
static int add_to_metadata(WavpackContext *wpc, void *data, uint32_t bcount,
                           unsigned char id);

// New for version 5.0, this function allows the application to store a file
// extension and a file_format identification. The extension would be used by
// the unpacker if the user had not specified the target filename, and
// specifically handles the case where the original file had the "wrong"
// extension for the file format (e.g., a Wave64 file having a "wav" extension)
// or an alternative (e.g., "bwf") or where the file format is not known.
// Specifying a file format besides the default WP_FORMAT_WAV will ensure that
// old decoders will not be able to see the non-wav wrapper provided with
// WavpackAddWrapper() (which they would end up putting on a file with a .wav
// extension).
...
void WavpackSetFileInformation(WavpackContext *wpc, char *file_extension,
  ...
  if (file_extension && strlen(file_extension) < sizeof(wpc->file_extension)) {
    add_to_metadata(wpc, file_extension, (uint32_t)strlen(file_extension),
                    ID_ALT_EXTENSION);
...
//
// The identities are provided in a NULL-terminated string (0x00 is not an
// allowed channel ID). The Microsoft channels may be provided as well (and will
// be checked) but it is really only neccessary to provide the "unknown"
// channels. Any truly unknown channels are indicated with a 0xFF.
//
// The channel IDs so far reserved are listed here:
//
// 0:           not allowed / terminator
// 1 - 18:      Microsoft standard channels
// 30, 31:      Stereo mix from RF64 (not really recommended, but RF64 specifies
// this) 33 - 44:     Core Audio channels (see Core Audio specification) 127 -
// 128:   Amio LeftHeight, Amio RightHeight 138 - 142:   Amio
// BottomFrontLeft/Center/Right, Amio ProximityLeft/Right 200 - 207:   Core
// Audio channels (see Core Audio specification) 221 - 224:   Core Audio
// channels 301 - 305 (offset by 80) 255:         Present but unknown or unused
// channel
//
...
// Legacy version of this function for compatibility with existing applications.
// Note that this version also generates older streams to be compatible with all
// decoders back to 4.0, but of course cannot be used with > 2^32 samples or
// non-Microsoft channels. The older stream version only differs in that it does
// not support the "mono optimization" feature where stereo blocks containing
// identical audio data in both channels are encoded in mono for better
// efficiency.

int WavpackSetConfiguration(WavpackContext *wpc, WavpackConfig *config,
...
int WavpackSetConfiguration64(WavpackContext *wpc, WavpackConfig *config,
  ...
  if ((config->qmode & QMODE_DSD_AUDIO) && config->bytes_per_sample == 1 &&
    ...
    // most options that don't apply to DSD we can simply ignore for now, but
    // NOT hybrid mode!
    ...
    // with DSD, very few PCM options work (or make sense), so only allow those
    // that do
    config->flags &=
        (CONFIG_HIGH_FLAG | CONFIG_MD5_CHECKSUM | CONFIG_PAIR_UNDEF_CHANS);
  ...
  if (!(flags & DSD_FLAG)) {
    if (config->float_norm_exp) {
      ...
      flags |= ((config->bytes_per_sample * 8) - config->bits_per_sample)
               << SHIFT_LSB;
    ...
    if (config->flags & CONFIG_HYBRID_FLAG) {
      ...
      if (config->flags & CONFIG_BITRATE_KBPS) {
        bps = (uint32_t)floor(config->bitrate * 256000.0 / config->sample_rate /
                                  config->num_channels +
                              0.5);
  ...
  // if a channel-identities string was specified, process that here, otherwise
  // all channels not present in the channel mask are considered "unassigned"

  if (chan_ids) {
    int lastchan = 0, mask_copy = chan_mask;

    if ((int)strlen((char *)chan_ids) >
        num_chans) { // can't be more than num channels!
      strcpy(wpc->error_message, "chan_ids longer than num channels!");
      return FALSE;
    }

    // skip past channels that are specified in the channel mask (no reason to
    // store those)

    while (*chan_ids)
      if (*chan_ids <= 32 && *chan_ids > lastchan &&
    ...
    // now scan the string for an actually defined channel (and don't store if
    // there aren't any)
  ...
  // This loop goes through all the channels and creates the Wavpack "streams"
  // for them to go in. A stream can hold either one or two channels, so we have
  // several rules to determine how many channels will go in each stream.
  ...
  for (wpc->current_stream = 0; num_chans; wpc->current_stream++) {
    ...
    wpc->streams = realloc(wpc->streams,
                           (wpc->current_stream + 1) * sizeof(wpc->streams[0]));
    wpc->streams[wpc->current_stream] = wps;
    CLEAR(*wps);

    // if there are any bits [still] set in the channel_mask, get the next one
    // or two IDs from there
    if (chan_mask)
    ...
    // if we have 2 channels, this is where we decide if we can combine them
    // into one stream:
    // 1. they are "unassigned" and we've been told to combine unassigned pairs,
    // or
    // 2. they appear together in the valid "pairings" list
    if (num_chans >= 2) {
      if ((config->flags & CONFIG_PAIR_UNDEF_CHANS) && left_chan_id == 0xff &&
          right_chan_id == 0xff)
        chans = 2;
      else
        for (i = 0; i < NUM_STEREO_PAIRS; ++i)
          if ((left_chan_id == stereo_pairs[i].a &&
               right_chan_id == stereo_pairs[i].b) ||
              (left_chan_id == stereo_pairs[i].b &&
               right_chan_id == stereo_pairs[i].a)) {
...
// This function allows setting the Core Audio File channel layout, many of
// which do not conform to the Microsoft ordering standard that Wavpack requires
// internally (at least for those channels present in the "channel mask"). In
// addition to the layout tag, this function allows a reordering string to be
// stored in the file to allow the unpacker to reorder the channels back to the
// specified layout (if it is aware of this feature and wants to restore the CAF
// order). The number of channels in the layout is specified in the lower nybble
// of the layout word, and if a reorder string is specified it must be that
// long. Note that all the reordering is actually done outside of this library,
// and that if reordering is done then the appropriate qmode bit must be set to
// ensure that any MD5 sum is stored with a new ID so that old decoders don't
// try to verify it (and to let the decoder know that a reorder might be
// required).
//
// Note: This function should only be used to encode Core Audio files in such a
// way that a verbatim archive can be created. Applications can just include the
// chan_ids parameter in the call to WavpackSetConfiguration64() if there are
// non-Microsoft channels to specify, or do nothing special if only Microsoft
// channels are present (the vast majority of cases).

int WavpackSetChannelLayout(WavpackContext *wpc, uint32_t layout_tag,
...
int WavpackPackInit(WavpackContext *wpc) {
  if (wpc->metabytes >
      16384) // 16384 bytes still leaves plenty of room for audio
    write_metadata_block(wpc); //  in this block (otherwise write a special one)

  // The default block size is a compromise. Longer blocks provide better
  // encoding efficiency, but longer blocks adversely affect memory requirements
  // and seeking performance. For WavPack version 5.0, the default block sizes
  // have been reduced by half from the previous version, but the difference in
  // encoding efficiency will generally be less than 0.1 percent.
  ...
  if (wpc->dsd_multiplier) {
    ...
    while (wpc->block_samples > 12000 &&
           wpc->block_samples * wpc->config.num_channels > 300000)
      wpc->block_samples /= 2;
  } else {
    ...
    while (wpc->block_samples > 12000 &&
           wpc->block_samples * wpc->config.num_channels > 75000)
      wpc->block_samples /= 2;
  ...
  for (wpc->current_stream = 0; wpc->current_stream < wpc->num_streams;
       wpc->current_stream++) {
    WavpackStream *wps = wpc->streams[wpc->current_stream];

    wps->sample_buffer =
        malloc(wpc->max_samples * (wps->wphdr.flags & MONO_FLAG ? 4 : 8));
...
static int create_riff_header(WavpackContext *wpc, int64_t total_samples,
                              void *outbuffer);

int WavpackPackSamples(WavpackContext *wpc, int32_t *sample_buffer,
  ...
  while (sample_count) {
    ...
    if (!wpc->riff_header_added && !wpc->riff_header_created &&
      ...
      if (!add_to_metadata(
              wpc, riff_header,
              create_riff_header(wpc, wpc->total_samples, riff_header),
              ID_RIFF_HEADER))
        return FALSE;
    }
    ...
    for (wpc->current_stream = 0; wpc->current_stream < wpc->num_streams;
      ...
      dptr = wps->sample_buffer +
             wpc->acc_samples * (wps->wphdr.flags & MONO_FLAG ? 1 : 2);
      sptr = source_pointer;
      cnt = samples_to_copy;

      // This code used to just copy the 32-bit samples regardless of the actual
      // size with the assumption that the caller had properly sign-extended the
      // values (if they were smaller than 32 bits). However, several people
      // have discovered that if the data isn't properly sign extended then ugly
      // things happen (e.g. CRC errors that show up only on decode). To prevent
      // this, we now explicitly sign-extend samples smaller than 32-bit when we
      // copy, and the performance hit from doing this is very small (generally
      // < 1%).
...
int WavpackStoreMD5Sum(WavpackContext *wpc, unsigned char data[16]) {
  return add_to_metadata(wpc, data, 16,
                         (wpc->config.qmode & 0xff) ? ID_ALT_MD5_CHECKSUM
                                                    : ID_MD5_CHECKSUM);
}
...
static int create_riff_header(WavpackContext *wpc, int64_t total_samples,
  ...
  if (format == 3 && wpc->config.float_norm_exp != 127) {
    strcpy(
        wpc->error_message,
        "can't create valid RIFF wav header for non-normalized floating data!");
    return FALSE;
  }
  ...
  total_riff_bytes = sizeof(riffhdr) + wavhdrsize + sizeof(datahdr) +
                     total_data_bytes + wpc->riff_trailer_bytes;
  if (do_rf64)
  ...
  if (do_rf64) {
    outptr =
        (char *)memcpy(outptr, &ds64hdr, sizeof(ds64hdr)) + sizeof(ds64hdr);
    outptr = (char *)memcpy(outptr, &ds64_chunk, sizeof(ds64_chunk)) +
             sizeof(ds64_chunk);
  }

  if (write_junk)
    outptr = (char *)memcpy(outptr, &junkchunk, sizeof(junkchunk)) +
             sizeof(junkchunk);
...
static int block_add_checksum(unsigned char *buffer_start,
                              unsigned char *buffer_end, int bytes);
...
static int pack_streams(WavpackContext *wpc, uint32_t block_samples) {
  ...
  // for calculating output (block) buffer size, first see if any streams are
  // stereo
  ...
  max_blocksize = block_samples * max_chans *
                  ((wpc->streams[0]->wphdr.flags & BYTES_STORED) + 1);

  // add margin based on how much "negative" compression is possible with
  // pathological audio

  if ((wpc->config.flags & CONFIG_FLOAT_DATA) &&
      !(wpc->config.flags & CONFIG_SKIP_WVX))
    max_blocksize += max_blocksize; // 100% margin for lossless float data
  else
    max_blocksize +=
        max_blocksize >> 2; // otherwise 25% margin for everything else

  max_blocksize +=
      wpc->metabytes + 1024; // finally, add metadata & another 1K margin
  ...
  for (wpc->current_stream = 0; wpc->current_stream < wpc->num_streams;
    ...
    if (result) {
      result =
          block_add_checksum(outbuff, outend, (flags & HYBRID_FLAG) ? 2 : 4);
    ...
    if (!result) {
      strcpy(wpc->error_message,
             "can't write WavPack data, disk probably full!");
      break;
    }
    ...
    if (out2buff) {
      ...
      WavpackNativeToLittleEndian((WavpackHeader *)out2buff,
                                  WavpackHeaderFormat);
    ...
    if (wpc->acc_samples != block_samples)
      memmove(wps->sample_buffer,
              wps->sample_buffer + block_samples * (flags & MONO_FLAG ? 1 : 2),
              (wpc->acc_samples - block_samples) * sizeof(int32_t) *
                  (flags & MONO_FLAG ? 1 : 2));
  }

  wpc->current_stream = 0;
  wpc->ave_block_samples =
      (wpc->ave_block_samples * 0x7 + block_samples + 0x4) >> 3;
...
void WavpackUpdateNumSamples(WavpackContext *wpc, void *first_block) {
  ...
  SET_TOTAL_SAMPLES(*(WavpackHeader *)first_block,
                    WavpackGetSampleIndex64(wpc));

  if (wpc->riff_header_created &&
      WavpackGetWrapperLocation(first_block, &wrapper_size)) {
    unsigned char riff_header[128];

    if (wrapper_size ==
        create_riff_header(wpc, WavpackGetSampleIndex64(wpc), riff_header))
      memcpy(WavpackGetWrapperLocation(first_block, NULL), riff_header,
             wrapper_size);
  }
...
static void *find_metadata(void *wavpack_block, int desired_id,
                           uint32_t *size) {
...
int copy_metadata(WavpackMetadata *wpmd, unsigned char *buffer_start,
  ...
  if (wpmd->data && wpmd->byte_length) {
    memcpy(buffer_start + (wpmd->byte_length > 510 ? 4 : 2), wpmd->data,
           wpmd->byte_length);
...
static int add_to_metadata(WavpackContext *wpc, void *data, uint32_t bcount,
  ...
  while (bcount) {
    ...
    if (bcount) {
      wpc->metadata = realloc(wpc->metadata,
                              (wpc->metacount + 1) * sizeof(WavpackMetadata));
...
static int write_metadata_block(WavpackContext *wpc) {
  ...
  if (wpc->metacount) {
    ...
    block_add_checksum((unsigned char *)block_buff,
                       (unsigned char *)block_buff + (block_size += 6), 4);
    WavpackNativeToLittleEndian((WavpackHeader *)block_buff,
                                WavpackHeaderFormat);
...
// These two functions add or update the block checksums that were introduced in
// WavPack 5.0. The presence of the checksum is indicated by a flag in the
// wavpack header (HAS_CHECKSUM) and the actual metadata item should be the last
// one in the block, and can be either 2 or 4 bytes. Of course, older versions
// of the decoder will simply ignore both of these.
...
static int block_add_checksum(unsigned char *buffer_start,
  ...
  if (bcount < sizeof(WavpackHeader) || (bcount & 1) ||
      buffer_start + bcount + 2 + bytes > buffer_end)
    return FALSE;
  ...
  while (wcount--)
    csum = (csum * 3) + *csptr++;
#else
  WavpackNativeToLittleEndian((WavpackHeader *)buffer_start,
                              WavpackHeaderFormat);
  ...
  WavpackLittleEndianToNative((WavpackHeader *)buffer_start,
                              WavpackHeaderFormat);
#endif
