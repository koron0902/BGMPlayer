...
#define USE_CTZ_OPTIMIZATION // use ctz intrinsic (or Windows equivalent) to
                             // count trailing ones
#else
#define USE_NEXT8_OPTIMIZATION // optimization using a table to count trailing
                               // ones
#endif

#define USE_BITMASK_TABLES // use tables instead of shifting for certain masking
                           // operations
...
int32_t FASTCALL get_word(WavpackStream *wps, int chan, int32_t *correction) {
  ...
  if (!(wps->w.c[0].median[0] & ~1) && !wps->w.holding_zero &&
      !wps->w.holding_one && !(wps->w.c[1].median[0] & ~1)) {
    uint32_t mask;
    int cbits;
  ...
  if (wps->w.holding_zero)
    ...
    {
      unsigned long res;
      _BitScanForward(&res, (unsigned long)~wps->wvbits.sr);
      ones_count = (uint32_t)res;
    }
#else
    ...
    if (ones_count >= LIMIT_ONES) {
      ...
      for (; ones_count < (LIMIT_ONES + 1) && getbit(&wps->wvbits);
           ++ones_count)
        ;
    ...
    if (next8 == 0xff) {
      ...
      for (ones_count = 8;
           ones_count < (LIMIT_ONES + 1) && getbit(&wps->wvbits); ++ones_count)
        ;
...
#else
    for (ones_count = 0; ones_count < (LIMIT_ONES + 1) && getbit(&wps->wvbits);
         ++ones_count)
      ;
...
int32_t get_words_lossless(WavpackStream *wps, int32_t *buffer,
  ...
  if (nsamples && !bs->ptr) {
    memset(buffer, 0,
           (wps->wphdr.flags & MONO_DATA) ? nsamples * 4 : nsamples * 8);
    return nsamples;
  }
  ...
  for (csamples = 0; csamples < nsamples; ++csamples) {
    ...
    if (wps->w.c[0].median[0] < 2 && !wps->w.holding_one &&
        wps->w.c[1].median[0] < 2) {
      uint32_t mask;
      int cbits;
    ...
    if (next8 == 0xff) {
      ...
      for (ones_count = 8; ones_count < (LIMIT_ONES + 1) && getbit(bs);
           ++ones_count)
        ;
...
#else
    for (ones_count = 0; ones_count < (LIMIT_ONES + 1) && getbit(bs);
         ++ones_count)
      ;
