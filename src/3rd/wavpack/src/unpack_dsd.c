...
int init_dsd_block(WavpackContext *wpc, WavpackMetadata *wpmd) {
  ...
  if (!wps->dsd.mode) {
    if (wps->dsd.endptr - wps->dsd.byteptr !=
        wps->wphdr.block_samples * (wps->wphdr.flags & MONO_DATA ? 1 : 2)) {
      return FALSE;
    }
...
int32_t unpack_dsd_samples(WavpackContext *wpc, int32_t *buffer,
  ...
  // don't attempt to decode past the end of the block, but watch out for
  // overflow!

  if (wps->sample_index + sample_count >
          GET_BLOCK_INDEX(wps->wphdr) + wps->wphdr.block_samples &&
      GET_BLOCK_INDEX(wps->wphdr) + wps->wphdr.block_samples -
              wps->sample_index <
          sample_count)
    sample_count = (uint32_t)(GET_BLOCK_INDEX(wps->wphdr) +
                              wps->wphdr.block_samples - wps->sample_index);

  if (GET_BLOCK_INDEX(wps->wphdr) > wps->sample_index ||
  ...
  if (wps->mute_error) {
    int samples_to_null;
    if (wpc->reduced_channels == 1 || wpc->config.num_channels == 1 ||
        (flags & MONO_FLAG))
...
static int init_dsd_block_fast(WavpackStream *wps, WavpackMetadata *wpmd) {
  ...
  wps->dsd.value_lookup = (unsigned char **)malloc(
      sizeof(*wps->dsd.value_lookup) * wps->dsd.history_bins);
  memset(wps->dsd.value_lookup, 0,
         sizeof(*wps->dsd.value_lookup) * wps->dsd.history_bins);
  wps->dsd.summed_probabilities = (int16_t(*)[256])malloc(
      sizeof(*wps->dsd.summed_probabilities) * wps->dsd.history_bins);
  wps->dsd.probabilities = (unsigned char(*)[256])malloc(
      sizeof(*wps->dsd.probabilities) * wps->dsd.history_bins);
  ...
  if (max_probability < 0xff) {
    ...
    unsigned char *outend =
        outptr + sizeof(*wps->dsd.probabilities) * wps->dsd.history_bins;
    ...
    if (outptr < outend ||
        (wps->dsd.byteptr < wps->dsd.endptr && *wps->dsd.byteptr++))
      return FALSE;
  } else if (wps->dsd.endptr - wps->dsd.byteptr >
             (int)sizeof(*wps->dsd.probabilities) * wps->dsd.history_bins) {
    memcpy(wps->dsd.probabilities, wps->dsd.byteptr,
           sizeof(*wps->dsd.probabilities) * wps->dsd.history_bins);
  ...
  for (wps->dsd.p0 = 0; wps->dsd.p0 < wps->dsd.history_bins; ++wps->dsd.p0) {
    ...
    for (sum_values = i = 0; i < 256; ++i)
      wps->dsd.summed_probabilities[wps->dsd.p0][i] = sum_values +=
          wps->dsd.probabilities[wps->dsd.p0][i];

    if (sum_values) {
      total_summed_probabilities += sum_values;
      vp = wps->dsd.value_lookup[wps->dsd.p0] =
          (unsigned char *)malloc(sum_values);
  ...
  if (wps->dsd.endptr - wps->dsd.byteptr < 4 ||
      total_summed_probabilities > wps->dsd.history_bins * 1280)
    return FALSE;
...
static int decode_fast(WavpackStream *wps, int32_t *output, int sample_count) {
  ...
  while (total_samples--) {
    ...
    mult = (wps->dsd.high - wps->dsd.low) /
           wps->dsd.summed_probabilities[wps->dsd.p0][255];
    ...
    if ((*output++ = code = wps->dsd.value_lookup[wps->dsd.p0][index]))
      wps->dsd.low +=
          wps->dsd.summed_probabilities[wps->dsd.p0][code - 1] * mult;

    wps->dsd.high =
        wps->dsd.low + wps->dsd.probabilities[wps->dsd.p0][code] * mult - 1;
    ...
    while (DSD_BYTE_READY(wps->dsd.high, wps->dsd.low) &&
           wps->dsd.byteptr < wps->dsd.endptr) {
      wps->dsd.value = (wps->dsd.value << 8) | *wps->dsd.byteptr++;
...
static int decode_high(WavpackStream *wps, int32_t *output, int sample_count) {
  int total_samples = sample_count,
      stereo = (wps->wphdr.flags & MONO_DATA) ? 0 : 1;
  ...
  while (total_samples--) {
    ...
    sp[0].value =
        sp[0].filter1 - sp[0].filter5 + ((sp[0].filter6 * sp[0].factor) >> 2);

    if (stereo)
      sp[1].value =
          sp[1].filter1 - sp[1].filter5 + ((sp[1].filter6 * sp[1].factor) >> 2);
    ...
    while (bitcount--) {
      ...
      uint32_t split =
          wps->dsd.low + ((wps->dsd.high - wps->dsd.low) >> 8) * (*pp >> 16);
      ...
      sp[0].factor +=
          (((sp[0].value ^ sp[0].filter0) >> 31) | 1) &
          ((sp[0].value ^ (sp[0].value - (sp[0].filter6 << 4))) >> 31);
      ...
      sp[1].factor +=
          (((sp[1].value ^ sp[1].filter0) >> 31) | 1) &
          ((sp[1].value ^ (sp[1].value - (sp[1].filter6 << 4))) >> 31);
    ...
    if (stereo) {
      wps->crc +=
          (wps->crc << 1) + (*output++ = wps->dsd.filters[1].byte & 0xff);
...
void *decimate_dsd_init(int num_channels) {
  DecimationContext *context =
      (DecimationContext *)malloc(sizeof(DecimationContext));
  ...
  context->chans =
      (DecimationChannel *)malloc(num_channels * sizeof(DecimationChannel));
