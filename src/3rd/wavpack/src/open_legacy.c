...
// This code provides an interface between the new reader callback mechanism
// that WavPack uses internally and the old reader callback functions that did
// not provide large file support.
...
static WavpackStreamReader64 trans_reader = {
    trans_read_bytes,  trans_write_bytes, trans_get_pos,
    trans_set_pos_abs, trans_set_pos_rel, trans_push_back_byte,
    trans_get_length,  trans_can_seek,    NULL,
    trans_close_stream};
...
// utilized and a translation layer is employed. It is provided as a
// compatibility function for existing applications. To ensure that streaming
// applications using this function continue to work, the OPEN_NO_CHECKSUM flag
// is forced on when the OPEN_STREAMING flag is set.
...
WavpackContext *WavpackOpenFileInputEx(WavpackStreamReader *reader, void *wv_id,
  ...
  // this prevents existing streaming applications from failing if they try to
  // pass in blocks that have been modified from the original (e.g., Matroska
  // blocks)
  ...
  if (wv_id) {
    trans_wv =
        (WavpackReaderTranslator *)malloc(sizeof(WavpackReaderTranslator));
  ...
  return WavpackOpenFileInputEx64(&trans_reader, trans_wv, trans_wvc, error,
                                  flags, norm_offset);
}
