...
void pack_init(WavpackContext *wpc) {
  ...
  if (wpc->config.flags & CONFIG_DYNAMIC_SHAPING)
    wps->dc.shaping_data =
        malloc(wpc->max_samples * sizeof(*wps->dc.shaping_data));
...
static void write_decorr_weights(WavpackStream *wps, WavpackMetadata *wpmd) {
  ...
  for (i = 0; i < wps->num_terms; ++i) {
    if (i < tcount) {
      dpp[i].weight_A =
          restore_weight(*byteptr++ = store_weight(dpp[i].weight_A));

      if (!(wps->wphdr.flags & MONO_DATA))
        dpp[i].weight_B =
            restore_weight(*byteptr++ = store_weight(dpp[i].weight_B));
    } else
  ...
  *byteptr++ = temp >> 8;
  ...
  if (wps->dc.shaping_delta[0] | wps->dc.shaping_delta[1]) {
    wps->dc.shaping_delta[0] =
        wp_exp2s(temp = wp_log2s(wps->dc.shaping_delta[0]));
    *byteptr++ = temp;
    *byteptr++ = temp >> 8;

    if (!(wps->wphdr.flags & MONO_DATA)) {
      wps->dc.shaping_delta[1] =
          wp_exp2s(temp = wp_log2s(wps->dc.shaping_delta[1]));
      *byteptr++ = temp;
      *byteptr++ = temp >> 8;
    }
  }
...
static void write_channel_info(WavpackContext *wpc, WavpackMetadata *wpmd) {
  ...
  if (wpc->num_streams > OLD_MAX_STREAMS) { // if > 8 streams, use 6 or 7 bytes
                                            // (breaks old decoders
    *byteptr++ = wpc->config.num_channels -
                 1; // that could only handle 8 streams) and allow (in theory)
    *byteptr++ = wpc->num_streams - 1; // up to 4096 channels
    *byteptr++ = (((wpc->num_streams - 1) >> 4) & 0xf0) |
                 (((wpc->config.num_channels - 1) >> 8) & 0xf);
    *byteptr++ = mask;
    *byteptr++ = (mask >> 8);
    *byteptr++ = (mask >> 16);

    if (mask &
        0xff000000) // this will break versions < 5.0, but is RF64-specific
      *byteptr++ = (mask >> 24);
...
static void write_channel_identities_info(WavpackContext *wpc,
...
static int scan_int32_data(WavpackStream *wps, int32_t *values,
                           int32_t num_values);
static void scan_int32_quick(WavpackStream *wps, int32_t *values,
                             int32_t num_values);
static void send_int32_data(WavpackStream *wps, int32_t *values,
                            int32_t num_values);
...
int pack_block(WavpackContext *wpc, int32_t *buffer) {
  ...
  // This is done first because this code can potentially change the size of the
  // block about to be encoded. This can happen because the dynamic noise
  // shaping algorithm wants to send a shorter block because the desired
  // noise-shaping profile is changing quickly. It can also be that the
  // --merge-blocks feature wants to create a longer block because it combines
  // areas with equal redundancy. These are not applicable for anything besides
  // the first stream of the file and they are not applicable with float data or
  // >24-bit data.

  if (!wpc->current_stream && !(flags & FLOAT_DATA) &&
      (flags & MAG_MASK) >> MAG_LSB < 24) {
    if ((wpc->config.flags & CONFIG_DYNAMIC_SHAPING) &&
    ...
    } else if (wpc->block_boundary &&
               sample_count >= (int32_t)wpc->block_boundary * 2) {
      int bc = sample_count / wpc->block_boundary,
          chans = (flags & MONO_DATA) ? 1 : 2;
  ...
  // This is where we handle any fixed shift which occurs when the integer size
  // does not evenly fit in bytes (like 12-bit or 20-bit) and is the same for
  // the entire file (not based on scanning)
  ...
  // The regular WavPack decorrelation and entropy encoding can handle up to
  // 24-bit integer data. If we have float data or integers larger than 24-bit,
  // then we have to potentially do extra processing. For lossy encoding, we can
  // simply convert this data in-place to 24-bit data and encode and sent that,
  // along with some metadata about how to restore the original format (even if
  // the restoration is not exact). However, for lossless operation we must make
  // a copy of the original data that will be used to create a "extension
  // stream" that will allow verbatim restoration of the original data. In the
  // hybrid mode that extension goes in the correction file, otherwise it goes
  // in the mail file.

  if ((flags & FLOAT_DATA) || (flags & MAG_MASK) >> MAG_LSB >=
                                  24) { // if float data or >24-bit integers...
    ...
    if ((!(flags & HYBRID_FLAG) || wpc->wvc_flag) &&
      ...
      orig_data =
          malloc(sizeof(f32) *
                 ((flags & MONO_DATA) ? sample_count : sample_count * 2));
      memcpy(orig_data, buffer,
             sizeof(f32) *
                 ((flags & MONO_DATA) ? sample_count : sample_count * 2));
      ...
      if (flags & FLOAT_DATA) { // if lossless float data come here
        ...
        if (!scan_float_data(wps, (f32 *)buffer,
                             (flags & MONO_DATA) ? sample_count
                                                 : sample_count * 2)) {
          free(orig_data);
          orig_data = NULL;
        }
      } else { // otherwise lossless > 24-bit integers
        if (!scan_int32_data(wps, buffer,
                             (flags & MONO_DATA) ? sample_count
                                                 : sample_count * 2)) {
          free(orig_data);
          orig_data = NULL;
      ...
      } else if (scan_int32_data(wps, buffer,
                                 (flags & MONO_DATA) ? sample_count
                                                     : sample_count * 2))
        wpc->lossy_blocks = TRUE;
    }

    // if there's any chance of magnitude change, clear the noise-shaping error
    // term and also reset the entropy encoder (which this does)

    wps->dc.error[0] = wps->dc.error[1] = 0;
    wps->num_terms = 0;
  }
  // if 24-bit integers or less we do a "quick" scan which just scans for
  // redundancy and does NOT set the flag's "magnitude" value
  else {
    scan_int32_quick(wps, buffer,
                     (flags & MONO_DATA) ? sample_count : sample_count * 2);

    if (wps->shift != wps->int32_zeros + wps->int32_ones +
                          wps->int32_dups) { // detect a change in any
                                             // redundancy shifting here
      wps->shift = wps->int32_zeros + wps->int32_ones + wps->int32_dups;
      wps->dc.error[0] = wps->dc.error[1] =
          0;              // on a change, clear the noise-shaping error term and
      wps->num_terms = 0; // also reset the entropy encoder (which this does)
    }
  }

  if ((wpc->config.flags & CONFIG_DYNAMIC_SHAPING) &&
      !dynamic_shaping_done) // calculate dynamic noise profile
    dynamic_noise_shaping(wpc, buffer, FALSE);

  // In some cases we need to start the decorrelation and entropy encoding from
  // scratch. This could be because we switched from stereo to mono encoding or
  // because the magnitude of the data changed, or just because this is the
  // first block.
  ...
  // actually pack the block here and return on an error (which pretty much can
  // only be a block buffer overrun)
  ...
  // potentially move any unused dynamic noise shaping profile data to use next
  // time
    ...
    if (wps->dc.shaping_samples != sample_count)
      memmove(wps->dc.shaping_data, wps->dc.shaping_data + sample_count,
              (wps->dc.shaping_samples - sample_count) *
                  sizeof(*wps->dc.shaping_data));

    wps->dc.shaping_samples -= sample_count;
  }

  // finally, if we're doing lossless float data or lossless >24-bit integers,
  // this is where we take the original data that we saved earlier and create
  // the "extension" stream containing the information required to refine the
  // "lossy" 24-bit data into the lossless original
  ...
  if (orig_data) {
    ...
    bs_open_write(&wps->wvxbits, cptr + 8,
                  wpc->wvc_flag ? wps->block2end : wps->blockend);

    if (flags & FLOAT_DATA)
      send_float_data(wps, (f32 *)orig_data,
                      (flags & MONO_DATA) ? sample_count : sample_count * 2);
    else
      send_int32_data(wps, orig_data,
                      (flags & MONO_DATA) ? sample_count : sample_count * 2);
...
static void scan_int32_quick(WavpackStream *wps, int32_t *values,
  ...
  wps->int32_sent_bits = wps->int32_zeros = wps->int32_ones = wps->int32_dups =
      0;
...
static int scan_int32_data(WavpackStream *wps, int32_t *values,
  ...
  if (((wps->wphdr.flags & MAG_MASK) >> MAG_LSB) > 23) {
    wps->int32_sent_bits =
        (unsigned char)(((wps->wphdr.flags & MAG_MASK) >> MAG_LSB) - 23);
...
uint32_t DECORR_MONO_BUFFER(int32_t *buffer, struct decorr_pass *decorr_passes,
                            int32_t num_terms, int32_t sample_count);

#ifdef OPT_ASM_X86
void decorr_stereo_pass(struct decorr_pass *dpp, int32_t *buffer,
                        int32_t sample_count);
void pack_decorr_stereo_pass_x86(struct decorr_pass *dpp, int32_t *buffer,
                                 int32_t sample_count);
...
// This macro controls the "repack" function where a block of samples will be
// repacked with fewer terms if a single residual exceeds the specified
// magnitude threshold.

#define REPACK_SAFE_NUM_TERMS                                                  \
  5 // 5 terms is always okay (and we truncate to this)
...
static int pack_samples(WavpackContext *wpc, int32_t *buffer) {
  ...
  if (repack_possible && !(flags & HYBRID_FLAG)) {
    saved_buffer =
        malloc(sample_count * sizeof(int32_t) * (flags & MONO_DATA ? 1 : 2));
    memcpy(saved_buffer, buffer,
           sample_count * sizeof(int32_t) * (flags & MONO_DATA ? 1 : 2));
  }

  // This code is written as a loop, but in the overwhelming majority of cases
  // it executes only once. If one of the higher modes is being used and a
  // residual exceeds a certain threshold, then the block will be repacked using
  // fewer decorrelation terms. Note that this has only been triggered by
  // pathological audio samples designed to trigger it...in practice this might
  // never happen. Note that this only applies to the "high" and "very high"
  // modes and only when packing directly (i.e. without the "extra" modes that
  // will have already checked magnitude).
  ...
  do {
    ...
    bs_open_write(&wps->wvbits,
                  wps->blockbuff + ((WavpackHeader *)wps->blockbuff)->ckSize +
                      12,
                  wps->blockend);
    ...
    if (wpc->wvc_flag) {
      ...
      bs_open_write(&wps->wvcbits,
                    wps->block2buff +
                        ((WavpackHeader *)wps->block2buff)->ckSize + 12,
                    wps->block2end);
    }
    ...
    if (!(flags & HYBRID_FLAG) && (flags & MONO_DATA)) {
      if (!wps->num_passes) {
        max_magnitude = DECORR_MONO_BUFFER(buffer, wps->decorr_passes,
                                           wps->num_terms, sample_count);
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        if (flags & HYBRID_SHAPE) {
          if (shaping_array)
            ...
            shaping_weight =
                (wps->dc.shaping_acc[0] += wps->dc.shaping_delta[0]) >> 16;
        ...
        for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++)
          if (dpp->term > MAX_TERM) {
            if (dpp->term & 1)
              ...
              dpp->samples_A[2] =
                  (3 * dpp->samples_A[0] - dpp->samples_A[1]) >> 1;

            code -= (dpp->aweight_A =
                         apply_weight(dpp->weight_A, dpp->samples_A[2]));
          } else
            code -= (dpp->aweight_A =
                         apply_weight(dpp->weight_A, dpp->samples_A[m]));
        ...
        while (--dpp >= wps->decorr_passes) {
          if (dpp->term > MAX_TERM) {
            ...
            dpp->samples_A[(m + dpp->term) & (MAX_TERM - 1)] =
                (code += dpp->aweight_A);
          }
        }
    ...
    /////////////////// handle the lossy/hybrid stereo mode ///////////////////
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        if (flags & HYBRID_SHAPE) {
          ...
          if (!shaping_array)
            shaping_weight =
                (wps->dc.shaping_acc[1] += wps->dc.shaping_delta[1]) >> 16;
          ...
          if (dpp->term > MAX_TERM) {
            if (dpp->term & 1) {
              ...
              dpp->samples_A[2] =
                  (3 * dpp->samples_A[0] - dpp->samples_A[1]) >> 1;
              dpp->samples_B[2] =
                  (3 * dpp->samples_B[0] - dpp->samples_B[1]) >> 1;
            }

            left -= (dpp->aweight_A =
                         apply_weight(dpp->weight_A, dpp->samples_A[2]));
            right -= (dpp->aweight_B =
                          apply_weight(dpp->weight_B, dpp->samples_B[2]));
          } else if (dpp->term > 0) {
            left -= (dpp->aweight_A =
                         apply_weight(dpp->weight_A, dpp->samples_A[m]));
            right -= (dpp->aweight_B =
                          apply_weight(dpp->weight_B, dpp->samples_B[m]));
          } else {
            if (dpp->term == -1)
            ...
            left -= (dpp->aweight_A =
                         apply_weight(dpp->weight_A, dpp->samples_A[0]));
            right -= (dpp->aweight_B =
                          apply_weight(dpp->weight_B, dpp->samples_B[0]));
          }

        max_magnitude |=
            (left < 0 ? ~left : left) | (right < 0 ? ~right : right);
        ...
        while (--dpp >= wps->decorr_passes)
          if (dpp->term > MAX_TERM) {
            ...
            update_weight_clip(dpp->weight_A, dpp->delta, dpp->samples_A[0],
                               left);
            update_weight_clip(dpp->weight_B, dpp->delta, dpp->samples_B[0],
                               right);
            dpp->samples_B[0] = (left += dpp->aweight_A);
            dpp->samples_A[0] = (right += dpp->aweight_B);
          }
    ...
    if (data_count) {
      if (data_count != (uint32_t)-1) {
        unsigned char *cptr =
            wps->blockbuff + ((WavpackHeader *)wps->blockbuff)->ckSize + 8;
    ...
    if (wpc->wvc_flag) {
      ...
      if (data_count && lossy) {
        if (data_count != (uint32_t)-1) {
          unsigned char *cptr =
              wps->block2buff + ((WavpackHeader *)wps->block2buff)->ckSize + 8;
    ...
    // we're done with the entire block, so now we check if our threshold for a
    // "repack" was hit

    if (repack_possible && wps->num_terms > REPACK_SAFE_NUM_TERMS &&
        (max_magnitude & repack_mask)) {
      *wps = saved_stream;
      wps->num_terms = REPACK_SAFE_NUM_TERMS;
      memcpy(wps->blockbuff, &wps->wphdr, sizeof(WavpackHeader));

      if (saved_buffer)
        memcpy(buffer, saved_buffer,
               sample_count * sizeof(int32_t) * (flags & MONO_DATA ? 1 : 2));

      if (flags & HYBRID_FLAG)
        crc = crc2 = 0xffffffff;
    } else {
      // if we actually did repack the block with fewer terms, we detect that
      // here and clean up so that we return to the original term
      // count...otherwise we just free the saved_buffer (if allocated) and
...
void decorr_stereo_pass(struct decorr_pass *dpp, int32_t *buffer,
  ...
  case 17:
    for (bptr = buffer; bptr < eptr; bptr += 2) {
      ...
      dpp->samples_A[1] = dpp->samples_A[0];
      bptr[0] = tmp =
          (dpp->samples_A[0] = bptr[0]) - apply_weight(dpp->weight_A, sam);
      update_weight(dpp->weight_A, dpp->delta, sam, tmp);

      sam = 2 * dpp->samples_B[0] - dpp->samples_B[1];
      dpp->samples_B[1] = dpp->samples_B[0];
      bptr[1] = tmp =
          (dpp->samples_B[0] = bptr[1]) - apply_weight(dpp->weight_B, sam);
  ...
  default:
    for (m = 0, k = dpp->term & (MAX_TERM - 1), bptr = buffer; bptr < eptr;
         bptr += 2) {
      int32_t sam, tmp;

      sam = dpp->samples_A[m];
      bptr[0] = tmp =
          (dpp->samples_A[k] = bptr[0]) - apply_weight(dpp->weight_A, sam);
      update_weight(dpp->weight_A, dpp->delta, sam, tmp);

      sam = dpp->samples_B[m];
      bptr[1] = tmp =
          (dpp->samples_B[k] = bptr[1]) - apply_weight(dpp->weight_B, sam);
      update_weight(dpp->weight_B, dpp->delta, sam, tmp);
  ...
  case -1:
    for (bptr = buffer; bptr < eptr; bptr += 2) {
      ...
      bptr[1] = tmp =
          (dpp->samples_A[0] = bptr[1]) - apply_weight(dpp->weight_B, sam_B);
  ...
  case -2:
    for (bptr = buffer; bptr < eptr; bptr += 2) {
      ...
      bptr[0] = tmp =
          (dpp->samples_B[0] = bptr[0]) - apply_weight(dpp->weight_A, sam_A);
      update_weight_clip(dpp->weight_A, dpp->delta, sam_A, tmp);
    }
