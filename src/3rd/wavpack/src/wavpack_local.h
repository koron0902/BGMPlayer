...
#define BITSTREAM_SHORTS // use 16-bit "shorts" for reading/writing bitstreams
                         // (instead of chars)
...
#define GET_BLOCK_INDEX(hdr)                                                   \
  ((int64_t)(hdr).block_index + ((int64_t)(hdr).block_index_u8 << 32))
...
// Macros to access the 40-bit total_samples field, which is complicated by the
// fact that all 1's in the lower 32 bits indicates "unknown" (regardless of
// upper 8 bits)

#define GET_TOTAL_SAMPLES(hdr)                                                 \
  (((hdr).total_samples == (uint32_t)-1)                                       \
       ? -1                                                                    \
       : (int64_t)(hdr).total_samples +                                        \
             ((int64_t)(hdr).total_samples_u8 << 32) - (hdr).total_samples_u8)
...
#define MAX_STREAM_VERS                                                        \
  0x410 // highest stream version we'll decode or encode
...
#define CONFIG_MERGE_BLOCKS                                                    \
  0x10000000 // merge blocks of equal redundancy (for lossyWAV)
#define CONFIG_PAIR_UNDEF_CHANS                                                \
  0x20000000 // encode undefined channels in stereo pairs
#define CONFIG_OPTIMIZE_MONO                                                   \
  0x80000000 // optimize for mono streams posing as stereo
...
// Note that this structure is directly accessed in assembly files, so modify
// with care
...
typedef struct {
  int32_t value, filter0, filter1, filter2, filter3, filter4, filter5, filter6,
      factor, byte;
} DSDfilters;
...
typedef struct {
  ...
  struct {
    unsigned char *byteptr, *endptr, (*probabilities)[256],
        **value_lookup, mode, ready;
...
typedef struct {
  ...
  int (*set_pos_rel)(void *id, int64_t delta,
                     int mode); // new signature for large files
  int (*push_back_byte)(void *id, int c);
  int64_t (*get_length)(void *id); // new signature for large files
  int (*can_seek)(void *id);
  int (*truncate_here)(
      void *id);          // new function to truncate file at current position
...
typedef struct {
  ...
  int wvc_flag, open_flags, norm_offset, reduced_channels, lossy_blocks,
      version_five;
  uint32_t block_samples, ave_block_samples, block_boundary, max_samples,
      acc_samples, riff_trailer_bytes;
  ...
  // these items were added in 5.0 to support alternate file types (especially
  // CAF & DSD)
...
// #define SKIP_DECORRELATION   // experimental switch to disable all
// decorrelation on encode
...
#define apply_weight_f(weight, sample)                                         \
  (((((sample & 0xffff) * weight) >> 9) +                                      \
    (((sample & ~0xffff) >> 9) * weight) + 1) >>                               \
   1)
#elif 1
#define apply_weight_f(weight, sample)                                         \
  ((int32_t)((weight * (int64_t)sample + 512) >> 10))
#else
#define apply_weight_f(weight, sample)                                         \
  ((int32_t)floor(((double)weight * sample + 512.0) / 1024.0))
#endif

#if 1 // PERFCOND - universal version that checks input magnitude or always uses
      // long version
#define apply_weight(weight, sample)                                           \
  (sample != (int16_t)sample ? apply_weight_f(weight, sample)                  \
...
#define update_weight(weight, delta, source, result)                           \
  if (source && result)                                                        \
    (source ^ result) < 0 ? (weight -= delta) : (weight += delta);
#endif

#define update_weight_clip(weight, delta, source, result)                      \
  if (source && result) {                                                      \
    const int32_t s = (source ^ result) >> 31;                                 \
    if ((weight = (weight ^ s) + (delta - s)) > 1024)                          \
      weight = 1024;                                                           \
    weight = (weight ^ s) - s;                                                 \
  }
...
int32_t unpack_samples(WavpackContext *wpc, int32_t *buffer,
...
void dynamic_noise_shaping(WavpackContext *wpc, int32_t *buffer,
                           int shortening_allowed);
void execute_stereo(WavpackContext *wpc, int32_t *samples, int no_history,
                    int do_samples);
void execute_mono(WavpackContext *wpc, int32_t *samples, int no_history,
                  int do_samples);
...
int32_t unpack_dsd_samples(WavpackContext *wpc, int32_t *buffer,
                           uint32_t sample_count);

void *decimate_dsd_init(int num_channels);
void decimate_dsd_reset(void *decimate_context);
void decimate_dsd_run(void *decimate_context, int32_t *samples,
                      int num_samples);
...
int unpack_cpu_has_feature_x86(int findex),
    pack_cpu_has_feature_x86(int findex);
...
int32_t unpack_samples3(WavpackContext *wpc, int32_t *buffer,
                        uint32_t sample_count);
...
#define count_bits(av)                                                         \
  ((av) < (1 << 8)                                                             \
       ...
       : ((av) < (1L << 16)                                                    \
              ? nbits_table[(av) >> 8] + 8                                     \
              : ((av) < (1L << 24) ? nbits_table[(av) >> 16] + 16              \
                                   : nbits_table[(av) >> 24] + 24)))
#endif
...
int32_t get_words_lossless(WavpackStream *wps, int32_t *buffer,
                           int32_t nsamples);
void flush_word(WavpackStream *wps);
int32_t nosend_word(WavpackStream *wps, int32_t value, int chan);
void scan_word(WavpackStream *wps, int32_t *samples, uint32_t num_samples,
               int dir);
...
#define OPEN_FILE_UTF8                                                         \
  0x80 // assume filenames are UTF-8 encoded, not ANSI (Windows only)
...
int WavpackGetTagItem(WavpackContext *wpc, const char *item, char *value,
                      int size);
int WavpackGetTagItemIndexed(WavpackContext *wpc, int index, char *item,
                             int size);
int WavpackGetNumBinaryTagItems(WavpackContext *wpc);
int WavpackGetBinaryTagItem(WavpackContext *wpc, const char *item, char *value,
                            int size);
int WavpackGetBinaryTagItemIndexed(WavpackContext *wpc, int index, char *item,
                                   int size);
int WavpackAppendTagItem(WavpackContext *wpc, const char *item,
                         const char *value, int vsize);
int WavpackAppendBinaryTagItem(WavpackContext *wpc, const char *item,
                               const char *value, int vsize);
