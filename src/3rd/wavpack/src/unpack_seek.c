...
static int64_t find_sample(WavpackContext *wpc, void *infile,
                           int64_t header_pos, int64_t sample);
...
int WavpackSeekSample64(WavpackContext *wpc, int64_t sample) {
...
#ifdef ENABLE_DSD
  if (wpc->decimation_context) { // the decimation code needs some context to be
                                 // sample accurate
    if (sample < 16) {
  ...
  if (!wps->wphdr.block_samples || !(wps->wphdr.flags & INITIAL_BLOCK) ||
      sample < GET_BLOCK_INDEX(wps->wphdr) ||
  ...
  if (!wps->blockbuff) {
    ...
    if (wpc->reader->read_bytes(
            wpc->wv_in, wps->blockbuff + sizeof(WavpackHeader),
            wps->wphdr.ckSize - 24) != wps->wphdr.ckSize - 24) {
      free_streams(wpc);
      return FALSE;
    }
    ...
    if (wpc->wvc_flag) {
      ...
      if (wpc->reader->read_bytes(
              wpc->wvc_in, wps->block2buff + sizeof(WavpackHeader),
              wps->wphdr.ckSize - 24) != wps->wphdr.ckSize - 24) {
        free_streams(wpc);
        return FALSE;
      }
  ...
  if (samples_to_decode) {
    buffer =
        (int32_t *)malloc(samples_to_decode * wpc->config.num_channels * 4);
...
static int64_t find_header(WavpackStreamReader64 *reader, void *id,
                           int64_t filepos, WavpackHeader *wphdr) {
  unsigned char *buffer = (unsigned char *)malloc(BUFSIZE), *sp = buffer,
                *ep = buffer;
  ...
  while (1) {
    ...
    while (sp + 32 <= ep)
      if (*sp++ == 'w' && *sp == 'v' && *++sp == 'p' && *++sp == 'k' &&
          !(*++sp & 1) && sp[2] < 16 && !sp[3] &&
          (sp[2] || sp[1] || *sp >= 24) && sp[5] == 4 &&
          sp[4] >= (MIN_STREAM_VERS & 0xff) &&
...
static int64_t find_sample(WavpackContext *wpc, void *infile,
  ...
  if (header_pos && wps->wphdr.block_samples) {
    ...
    } else if (GET_BLOCK_INDEX(wps->wphdr) + wps->wphdr.block_samples <=
               sample) {
