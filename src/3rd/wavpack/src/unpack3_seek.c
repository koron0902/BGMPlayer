...
static void *unpack_restore(WavpackStream3 *wps, void *source,
                            int keep_resources);
...
int seek_sample3(WavpackContext *wpc, uint32_t desired_index) {
  ...
  if (wps->index_points[points_index].saved)
    if (wps->index_points[points_index].sample_index > wps->sample_index ||
      ...
      unpack_restore(wps, wps->unpack_data + points_index * wps->unpack_size,
                     TRUE);
    }

  if (desired_index > wps->sample_index) {
    int32_t *buffer =
        (int32_t *)malloc(1024 * (wps->wphdr.flags & MONO_FLAG ? 4 : 8));
