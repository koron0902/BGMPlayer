...
uint32_t WavpackUnpackSamples(WavpackContext *wpc, int32_t *buffer,
                              uint32_t samples) {
  WavpackStream *wps =
      wpc->streams ? wpc->streams[wpc->current_stream = 0] : NULL;
  ...
  while (samples) {
    ...
    // if the current block has no audio, or it's not the first block of a
    // multichannel sequence, or the sample we're on is past the last sample in
    // this block...we need to free up the streams and read the next block

    if (!wps->wphdr.block_samples || !(wps->wphdr.flags & INITIAL_BLOCK) ||
        wps->sample_index >=
            GET_BLOCK_INDEX(wps->wphdr) + wps->wphdr.block_samples) {
      ...
      if (wpc->open_flags & OPEN_STREAMING)
        ...
        SET_BLOCK_INDEX(wps->wphdr,
                        GET_BLOCK_INDEX(wps->wphdr) - wpc->initial_index);

      memcpy(wps->blockbuff, &wps->wphdr, 32);
      wps->init_done =
          FALSE; // we have not yet called unpack_init() for this block

      // if this block has audio, but not the sample index we were expecting,
      // flag an error
      ...
      // if this block has audio, and we're in hybrid lossless mode, read the
      // matching wvc block

      if (wps->wphdr.block_samples && wpc->wvc_flag)
        read_wvc_block(wpc);

      // if the block does NOT have any audio, call unpack_init() to process
      // non-audio stuff
    ...
    // if the current block has no audio, or it's not the first block of a
    // multichannel sequence, or the sample we're on is past the last sample in
    // this block...we need to loop back and read the next block
    ...
    // There seems to be some missing data, like a block was corrupted or
    // something. If it's not too much data, just fill in with silence here and
    // loop back.
    ...
    if (wps->sample_index < GET_BLOCK_INDEX(wps->wphdr)) {
      ...
      samples_to_unpack =
          (uint32_t)(GET_BLOCK_INDEX(wps->wphdr) - wps->sample_index);
      ...
      samples_to_unpack *=
          (wpc->reduced_channels ? wpc->reduced_channels : num_channels);
    ...
    // calculate number of samples to process from this block, then initialize
    // the decoder for this block if we haven't already

    samples_to_unpack =
        (uint32_t)(GET_BLOCK_INDEX(wps->wphdr) + wps->wphdr.block_samples -
                   wps->sample_index);
    ...
    // if this block is not the final block of a multichannel sequence (and
    // we're not truncating to stereo), then enter this conditional
    // block...otherwise we just unpack the samples directly

    if (!wpc->reduced_channels && !(wps->wphdr.flags & FINAL_BLOCK)) {
      int32_t *temp_buffer = (int32_t *)malloc(samples_to_unpack * 8), *src,
              *dst;
      int offset =
          0; // offset to next channel in sequence (0 to num_channels - 1)
      uint32_t samcnt;

      // since we are getting samples from multiple bocks in a multichannel
      // sequence, we must allocate a temporary buffer to unpack to so that we
      // can re-interleave the samples
      ...
      while (1) {
        ...
        // if the stream has not been allocated and corresponding block read, do
        // that here...

        if (wpc->current_stream == wpc->num_streams) {
          wpc->streams = (WavpackStream **)realloc(
              wpc->streams, (wpc->num_streams + 1) * sizeof(wpc->streams[0]));

          if (!wpc->streams)
            break;

          wps = wpc->streams[wpc->num_streams++] =
              (WavpackStream *)malloc(sizeof(WavpackStream));
          ...
          // render corrupt blocks harmless
          if (!WavpackVerifySingleBlock(
                  wps->blockbuff, !(wpc->open_flags & OPEN_NO_CHECKSUM))) {
            wps->wphdr.ckSize = sizeof(WavpackHeader) - 8;
            wps->wphdr.block_samples = 0;
            memcpy(wps->blockbuff, &wps->wphdr, 32);
          }

          // potentially adjusting block_index must be done AFTER verifying
          // block
          ...
          // if this block has audio, and we're in hybrid lossless mode, read
          // the matching wvc block
          ...
          // unpack the correct number of samples (either mono or stereo) into
          // the temp buffer
        ...
        // if the block is mono, copy the samples from the single channel into
        // the destination using num_channels as the stride
        ...
        // if the block is stereo, and we don't have room for two more channels,
        // just copy one and flag an error
        ...
        // check several clues that we're done with this set of blocks and exit
        // if we are; else do next stream

        if ((wps->wphdr.flags & FINAL_BLOCK) ||
            wpc->current_stream == wpc->max_streams - 1 ||
            offset == num_channels)
          break;
        else
          wpc->current_stream++;
      ...
      // if we didn't get all the channels we expected, mute the buffer and flag
      // an error
      ...
      // go back to the first stream (we're going to leave them all loaded for
      // now because they might have more samples) and free the temp buffer

      wps = wpc->streams[wpc->current_stream = 0];
      free(temp_buffer);
    }
    // catch the error situation where we have only one channel but run into a
    // stereo block (this avoids overwriting the caller's buffer)
    else if (!(wps->wphdr.flags & MONO_FLAG) &&
             (num_channels == 1 || wpc->reduced_channels == 1)) {
      ...
      if (check_crc_error(wpc)) {
        ...
        if (wps->blockbuff && wpc->reader->can_seek(wpc->wv_in)) {
          ...
          wpc->reader->set_pos_rel(wpc->wv_in,
                                   (rseek > 16384) ? -16384 : -rseek, SEEK_CUR);
        }

        if (wpc->wvc_flag && wps->block2buff &&
            wpc->reader->can_seek(wpc->wvc_in)) {
          int32_t rseek = ((WavpackHeader *)wps->block2buff)->ckSize / 3;
          wpc->reader->set_pos_rel(wpc->wvc_in,
                                   (rseek > 16384) ? -16384 : -rseek, SEEK_CUR);
        }
