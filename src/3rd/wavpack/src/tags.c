...
int load_tag(WavpackContext *wpc) {
  ...
  // This is a loop because we can try up to three times to look for an APEv2
  // tag. In order, we look:
  //
  //  1. At the end of the file for a APEv2 footer (this is the preferred
  //  location)
  //  2. If there's instead an ID3v1 tag at the end of the file, try looking for
  //  an APEv2 footer right before that
  //  3. If all else fails, look for an APEv2 header the the beginning of the
  //  file (use is strongly discouraged)

  while (1) {

    // seek based on specific location that we are looking for tag (see above
    // list)

    if (m_tag->tag_begins_file) // case #3
      wpc->reader->set_pos_abs(wpc->wv_in, 0);
    else if (m_tag->id3_tag.tag_id[0] == 'T') // case #2
      wpc->reader->set_pos_rel(
          wpc->wv_in, -(int32_t)(sizeof(APE_Tag_Hdr) + sizeof(ID3_Tag)),
          SEEK_END);
    else // case #1
      wpc->reader->set_pos_rel(wpc->wv_in, -(int32_t)sizeof(APE_Tag_Hdr),
                               SEEK_END);

    // read a possible APEv2 tag header/footer and see if there's one there...

    if (wpc->reader->read_bytes(wpc->wv_in, &m_tag->ape_tag_hdr,
                                sizeof(APE_Tag_Hdr)) == sizeof(APE_Tag_Hdr) &&
      ...
      if (m_tag->ape_tag_hdr.version == 2000 && m_tag->ape_tag_hdr.item_count &&
          ...
          (m_tag->ape_tag_data =
               (unsigned char *)malloc(m_tag->ape_tag_hdr.length)) != NULL) {

        ape_tag_items = m_tag->ape_tag_hdr.item_count;
        ape_tag_length = m_tag->ape_tag_hdr.length;

        // If this is a APEv2 footer (which is normal if we are searching at the
        // end of the file)...
        ...
        if (!(m_tag->ape_tag_hdr.flags & APE_TAG_THIS_IS_HEADER)) {
          ...
          // if the footer claims there is a header present also, we will read
          // that and use it instead of the footer (after verifying it, of
          // course) for enhanced robustness
          ...
          if (m_tag->ape_tag_hdr.flags & APE_TAG_CONTAINS_HEADER) {
            ...
            WavpackLittleEndianToNative(&m_tag->ape_tag_hdr,
                                        APE_Tag_Hdr_Format);

            if (m_tag->ape_tag_hdr.version != 2000 ||
                m_tag->ape_tag_hdr.item_count != ape_tag_items ||
    ...
    // we come here if the search for the APEv2 tag failed (otherwise we would
    // have returned with it)

    if (m_tag->id3_tag.tag_id[0] ==
        'T') { // settle for the ID3v1 tag that we found
      CLEAR(m_tag->ape_tag_hdr);
      return TRUE;
    }

    // if this was the search for the APEv2 tag at the beginning of the file
    // (which is our last resort) then we have nothing, so return failure
    ...
    // If we get here, then we have failed the first APEv2 tag search (at end of
    // file) and so now we look for an ID3v1 tag at the same position. If that
    // succeeds, then we'll loop back and look for an APEv2 tag immediately
    // before the ID3v1 tag, otherwise our last resort is to look for an APEv2
    // tag at the beginning of the file. These are strongly discouraged (and not
    // editable) but they have been seen in the wild so we attempt to handle
    // them here (at least well enough to allow a proper transcoding).

    m_tag->tag_file_pos = -(int32_t)sizeof(ID3_Tag);
    wpc->reader->set_pos_rel(wpc->wv_in, m_tag->tag_file_pos, SEEK_END);

    if (wpc->reader->read_bytes(wpc->wv_in, &m_tag->id3_tag, sizeof(ID3_Tag)) !=
            sizeof(ID3_Tag) ||
        strncmp(m_tag->id3_tag.tag_id, "TAG", 3)) {
      m_tag->tag_begins_file =
          1; // failed ID3v1, so look for APEv2 at beginning of file
      CLEAR(m_tag->id3_tag);
    }
  }
}
...
// Return FALSE if a valid APEv2 tag was only found at the beginning of the file
// (these are read-only because they cannot be edited without possibly shifting
// the entire file)
