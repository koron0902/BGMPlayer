...
static int bs_open_read3(Bitstream3 *bs, WavpackStreamReader64 *reader,
                         void *id);
...
#define apply_weight_n(bits, weight, sample)                                   \
  ((weight * sample + (1 << (bits - 1))) >> bits)
...
#define apply_weight24(weight, sample)                                         \
  (((((sample & 0xffff) * weight) >> 7) +                                      \
    (((sample & ~0xffff) >> 7) * weight) + 1) >>                               \
   1)
...
static const signed char extreme_terms[] = {1, 1,  1, 2, 4, -1, 1, 2, 3,
                                            6, -2, 8, 5, 7, 4,  1, 2, 3};
...
void unpack_init3(WavpackStream3 *wps) {
  ...
  if (flags & EXTREME_DECORR) {
    for (dpp = wps->decorr_passes, ti = 0; ti < sizeof(extreme_terms); ti++)
      if (extreme_terms[sizeof(extreme_terms) - ti - 1] > 0 ||
          (flags & CROSS_DECORR))
    ...
    for (dpp = wps->decorr_passes, ti = 0; ti < sizeof(default_terms); ti++)
      if (default_terms[sizeof(default_terms) - ti - 1] > 0 ||
          (flags & CROSS_DECORR))
...
static int unpack_size(WavpackStream3 *wps) {
  ...
  if (flags & OVER_20)
    byte_sum +=
        sizeof(wps->dc.last_extra_bits) + sizeof(wps->dc.extra_bits_count);
...
static int32_t FASTCALL get_word4(WavpackStream3 *wps, int chan,
                                  int32_t *correction);

int32_t unpack_samples3(WavpackContext *wpc, int32_t *buffer,
                        uint32_t sample_count) {
  WavpackStream3 *wps = (WavpackStream3 *)wpc->stream3;
  int shift = wps->wphdr.shift, flags = wps->wphdr.flags, min_weight = 0,
      m = wps->dc.m, tcount;
#ifndef NO_SEEKING
  int points_index =
      wps->sample_index / (((uint32_t)wpc->total_samples >> 8) + 1);
#endif
  ...
  if (!wps->index_points[points_index].saved) {
    ...
    if (!wps->unpack_data)
      wps->unpack_data =
          (unsigned char *)malloc(256 * (wps->unpack_size = unpack_size(wps)));
  ...
  if (wps->wphdr.version == 3 && !wps->wphdr.bits && (flags & MONO_FLAG)) {
    if (flags & FAST_FLAG) {
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        if (flags & EXTREME_DECORR)
          for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--;
               dpp++) {
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        temp = sample[0][0] + ((sample[0][1] * weight[0][0] + 128) >> 8) +
               read_word;
        ...
        if (flags & OVER_20) {
          ...
          crc = crc * 3 + (*bptr++ = temp =
                               wps->dc.last_extra_bits + (sample[0][0] << 4));
        } else {
  ...
  else if (wps->wphdr.version == 3 && !wps->wphdr.bits &&
           !(flags & MONO_FLAG)) {
    ...
    if (flags & FAST_FLAG) {
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        if (flags & CROSS_DECORR) {
          ...
          if (flags & NEW_HIGH_FLAG) {
            ...
            if (right_level > left_level) {
              if (left_level + right_level < sum_level + diff_level &&
                  right_level < diff_level) {
        ...
        if (flags & EXTREME_DECORR) {
          for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--;
            ...
            if (dpp->term > 0) {
              ...
              left2 =
                  left + apply_weight_n(9, dpp->weight_A, dpp->samples_A[0]);
              ...
              right2 =
                  right + apply_weight_n(9, dpp->weight_A, dpp->samples_A[0]);
          ...
          for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--;
            ...
            if (dpp->term > 0) {
              ...
              left2 =
                  left + apply_weight_n(8, dpp->weight_A, dpp->samples_A[0]);
              ...
              right2 =
                  right + apply_weight_n(8, dpp->weight_A, dpp->samples_A[0]);
        ...
        if (flags & OVER_20) {
          ...
          crc = crc * 3 +
                (*bptr++ = left = (left << 4) + (wps->dc.last_extra_bits >> 4));
          crc = crc * 3 + (*bptr++ = right =
                               (right << 4) + (wps->dc.last_extra_bits & 0xf));
        } else {
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        left2 =
            sample[0][0] + ((sample[0][1] * weight[0][0] + 128) >> 8) + left;
        right2 =
            sample[1][0] + ((sample[1][1] * weight[1][0] + 128) >> 8) + right;
        ...
        if (flags & OVER_20) {
          ...
          crc = crc * 3 + (*bptr++ = left2 = (sample[0][0] << 4) +
                                             (wps->dc.last_extra_bits >> 4));
          crc = crc * 3 + (*bptr++ = right2 = (sample[1][0] << 4) +
                                              (wps->dc.last_extra_bits & 0xf));
        } else {
  ...
  //////////////// handle version 3 lossy/hybrid mono data //////////////////
    ...
    if (flags & FAST_FLAG)
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        read_word = (flags & NEW_HIGH_FLAG) ? get_word4(wps, 0, correction)
                                            : get_word3(wps, 0);
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        new_sample = sample[0][0] + ((sample[0][1] * weight[0][0] + 128) >> 8) +
                     read_word;
  ...
  //////////////// handle version 3 lossy/hybrid stereo data ////////////////
    ...
    if (flags & FAST_FLAG)
      ...
      for (bptr = buffer, i = 0; i < sample_count; ++i) {
        ...
        new_sample = sample[1][0] + ((sample[1][1] * weight[1][0] + 128) >> 8) +
                     read_word;
  ...
  if (mute_block)
    memset(buffer, 0,
           sizeof(*buffer) * sample_count * ((flags & MONO_FLAG) ? 1 : 2));
#endif

  if (i && (wps->sample_index += i) == wpc->total_samples) {

    if (wps->wphdr.version == 3 &&
        crc != (wpc->wvc_flag ? wps->wphdr.crc2 : wps->wphdr.crc))
      wpc->crc_errors++;
    ...
    if (wpc->open_flags & OPEN_WRAPPER) {
      ...
      if (bs_unused_bytes(&wps->wvbits)) {
        ...
        memcpy(wpc->wrapper_data + wpc->wrapper_bytes,
               bs_unused_data(&wps->wvbits), bs_unused_bytes(&wps->wvbits));
        wpc->wrapper_bytes += bs_unused_bytes(&wps->wvbits);
      }
      ...
      while (1) {
        ...
        wpc->wrapper_data = (unsigned char *)realloc(
            wpc->wrapper_data, wpc->wrapper_bytes + bcount);
      ...
      if (wpc->wrapper_bytes > 16) {
        ...
        if (c == 16) {
          memmove(wpc->wrapper_data, wpc->wrapper_data + 16,
...
static int32_t FASTCALL get_word1(WavpackStream3 *wps, int chan) {
  ...
  if ((wps->wphdr.flags & EXTREME_DECORR) && !(wps->wphdr.flags & OVER_20)) {
    if (wps->w1.zeros_acc) {
      if (--wps->w1.zeros_acc)
        return 0;
    } else if (wps->w1.ave_level[0][0] < 0x20 &&
               wps->w1.ave_level[0][1] < 0x20) {
      int32_t mask;
      int cbits;
  ...
  k = (wps->w1.ave_level[0][chan] + (wps->w1.ave_level[0][chan] >> 3) + 0x40) >>
      7;
  k = count_bits(k);
  ...
  if (ones_count == 0) {
    ...
    tmp1 = bitset[k];
    k = (wps->w1.ave_level[1][chan] + (wps->w1.ave_level[1][chan] >> 4) +
         0x20) >>
        6;
    k = count_bits(k);
...
static int32_t FASTCALL get_word3(WavpackStream3 *wps, int chan) {
  ...
  if (wps->wphdr.bits && dbits > wps->wphdr.bits) {
    ...
    if (value & bitset[wps->wphdr.bits - 1])
      return -(int32_t)(value & bitmask[wps->wphdr.bits])
             << (dbits - wps->wphdr.bits);
    else
      return ((value & bitmask[wps->wphdr.bits - 1]) |
              bitset[wps->wphdr.bits - 1])
             << (dbits - wps->wphdr.bits);
  } else {
...
static int32_t FASTCALL get_word4(WavpackStream3 *wps, int chan,
  ...
  if (!chan) {
    ...
    if (wps->wphdr.flags & MONO_FLAG) {
      wps->w4.bits_acc[0] += wps->w4.bitrate + wp3_log2(wps->w4.fast_level[0]) -
                             wp3_log2(wps->w4.slow_level[0]) + (3 << 8);
      ...
      wps->w4.bits_acc[0] += wps->w4.bitrate - balance +
                             wp3_log2(wps->w4.fast_level[0]) - slow_log_0 +
                             (3 << 8);
      wps->w4.bits_acc[1] += wps->w4.bitrate + balance +
                             wp3_log2(wps->w4.fast_level[1]) - slow_log_1 +
                             (3 << 8);
  ...
  if (!base) {
    if (ones_count)
      high = low = mid =
          (getbit(&wps->wvbits)) ? -(int32_t)ones_count : ones_count;
    else
      high = low = mid = 0;
  } else {
