...
static int get_ape_tag_item(M_Tag *m_tag, const char *item, char *value,
                            int size, int type);
static int get_id3_tag_item(M_Tag *m_tag, const char *item, char *value,
                            int size);
static int get_ape_tag_item_indexed(M_Tag *m_tag, int index, char *item,
                                    int size, int type);
static int get_id3_tag_item_indexed(M_Tag *m_tag, int index, char *item,
                                    int size);
static int append_ape_tag_item(WavpackContext *wpc, const char *item,
                               const char *value, int vsize, int type);
...
// Count and return the total number of binary tag items in the specified file.
// This applies only to APEv2 tags and was implemented as a separate function to
// avoid breaking the old API.
...
// Delete the specified tag item from the APEv2 tag on the specified WavPack
// file (fields cannot be deleted from ID3v1 tags). A return value of TRUE
// indicates that the item was found and successfully deleted.
...
int WavpackDeleteTagItem(WavpackContext *wpc, const char *item) {
  ...
  if (m_tag->ape_tag_hdr.ID[0] == 'A') {
    ...
    for (i = 0; i < m_tag->ape_tag_hdr.item_count; ++i) {
      ...
      vsize = p[0] + (p[1] << 8) + (p[2] << 16) + (p[3] << 24);
      p += 8; // skip flags because we don't need them
      for (isize = 0; p[isize] && p + isize < q; ++isize)
        ;

      if (vsize < 0 || vsize > m_tag->ape_tag_hdr.length ||
          p + isize + vsize + 1 > q)
        break;
      ...
      if (isize && vsize && !stricmp(item, (char *)p)) {
        ...
        m_tag->ape_tag_hdr.length =
            (int32_t)(d - m_tag->ape_tag_data) + sizeof(APE_Tag_Hdr);
...
static int get_id3_tag_item(M_Tag *m_tag, const char *item, char *value,
  ...
  else if (!stricmp(item, "track") && m_tag->id3_tag.comment[29] &&
           !m_tag->id3_tag.comment[28])
...
static int get_ape_tag_item_indexed(M_Tag *m_tag, int index, char *item,
  ...
  for (i = 0; i < m_tag->ape_tag_hdr.item_count && index >= 0 && q - p > 8;
       ++i) {
    int vsize, flags, isize;
...
static int get_id3_tag_item_indexed(M_Tag *m_tag, int index, char *item,
  ...
  if (tagdata(m_tag->id3_tag.title, sizeof(m_tag->id3_tag.title)) && !index--)
    strcpy(lvalue, "Title");
  else if (tagdata(m_tag->id3_tag.artist, sizeof(m_tag->id3_tag.artist)) &&
           !index--)
    strcpy(lvalue, "Artist");
  else if (tagdata(m_tag->id3_tag.album, sizeof(m_tag->id3_tag.album)) &&
           !index--)
    strcpy(lvalue, "Album");
  else if (tagdata(m_tag->id3_tag.year, sizeof(m_tag->id3_tag.year)) &&
           !index--)
    strcpy(lvalue, "Year");
  else if (tagdata(m_tag->id3_tag.comment, sizeof(m_tag->id3_tag.comment)) &&
           !index--)
    strcpy(lvalue, "Comment");
  else if (m_tag->id3_tag.comment[29] && !m_tag->id3_tag.comment[28] &&
           !index--)
    strcpy(lvalue, "Track");
  else
    return 0;
...
static int append_ape_tag_item(WavpackContext *wpc, const char *item,
  ...
  if (!m_tag->ape_tag_hdr.ID[0]) {
    ...
    m_tag->ape_tag_hdr.flags =
        APE_TAG_CONTAINS_HEADER; // we will include header on tags we originate
  }
  ...
  if (m_tag->ape_tag_hdr.ID[0] == 'A') {
    ...
    p = m_tag->ape_tag_data = (unsigned char *)realloc(
        m_tag->ape_tag_data, m_tag->ape_tag_hdr.length);
...
// Append the stored APEv2 tag to the file being created using the "blockout"
// function callback.
...
// Write the [potentially] edited tag to the existing WavPack file using the
...
static int write_tag_reader(WavpackContext *wpc) {
  ...
  // before we write an edited (or new) tag into an existing file, make sure
  // it's safe and possible

  if (m_tag->tag_begins_file) {
    strcpy(wpc->error_message,
           "can't edit tags located at the beginning of files!");
    return FALSE;
  }
  ...
  if (result && tag_size) {
    if (m_tag->ape_tag_hdr.flags & APE_TAG_CONTAINS_HEADER) {
      m_tag->ape_tag_hdr.flags |= APE_TAG_THIS_IS_HEADER;
      WavpackNativeToLittleEndian(&m_tag->ape_tag_hdr, APE_Tag_Hdr_Format);
      result = (wpc->reader->write_bytes(wpc->wv_in, &m_tag->ape_tag_hdr,
                                         sizeof(m_tag->ape_tag_hdr)) ==
                sizeof(m_tag->ape_tag_hdr));
      WavpackLittleEndianToNative(&m_tag->ape_tag_hdr, APE_Tag_Hdr_Format);
    }

    result = (wpc->reader->write_bytes(wpc->wv_in, m_tag->ape_tag_data,
                                       m_tag->ape_tag_hdr.length -
                                           sizeof(m_tag->ape_tag_hdr)) ==
              sizeof(m_tag->ape_tag_hdr));
    m_tag->ape_tag_hdr.flags &= ~APE_TAG_THIS_IS_HEADER; // this is NOT header
