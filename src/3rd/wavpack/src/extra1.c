...
void PACK_DECORR_MONO_PASS_CONT(int32_t *out_buffer, int32_t *in_buffer,
                                struct decorr_pass *dpp, int32_t sample_count);
#endif
...
static void decorr_mono_pass(int32_t *in_samples, int32_t *out_samples,
                             uint32_t num_samples, struct decorr_pass *dpp,
                             int dir) {
...
static void decorr_mono_buffer(int32_t *samples, int32_t *outsamples,
  ...
  dp.term = term;
  dp.delta = pre_delta;
  decorr_mono_pass(samples, outsamples, num_samples > 2048 ? 2048 : num_samples,
                   &dp, -1);
  dp.delta = delta;
...
static void recurse_mono(WavpackContext *wpc, WavpackExtraInfo *info, int depth,
  ...
  for (term = 1; term <= 18; ++term) {
    ...
    decorr_mono_buffer(samples, outsamples, wps->wphdr.block_samples, info->dps,
                       depth);
...
static void delta_mono(WavpackContext *wpc, WavpackExtraInfo *info) {
  ...
  for (d = delta - 1; d >= 0; --d) {
    ...
    for (i = 0; i < info->nterms && wps->decorr_passes[i].term; ++i) {
      ...
      info->dps[i].delta = d;
      decorr_mono_buffer(info->sampleptrs[i], info->sampleptrs[i + 1],
                         wps->wphdr.block_samples, info->dps, i);
    }

    bits = LOG2BUFFER(info->sampleptrs[i], wps->wphdr.block_samples,
                      info->log_limit);
  ...
  while (reversed) {
    ...
    for (ri = 0; ri < info->nterms && wps->decorr_passes[ri].term; ++ri) {
      ...
      if (wps->decorr_passes[ri].term == wps->decorr_passes[ri + 1].term) {
        decorr_mono_buffer(info->sampleptrs[ri], info->sampleptrs[ri + 1],
                           wps->wphdr.block_samples, info->dps, ri);
        continue;
      }
...
static void analyze_mono(WavpackContext *wpc, int32_t *samples,
                         int do_samples) {
  WavpackStream *wps = wpc->streams[wpc->current_stream];
  ...
  for (i = 0; i < info.nterms && info.dps[i].term; ++i)
    decorr_mono_pass(info.sampleptrs[i], info.sampleptrs[i + 1],
                     wps->wphdr.block_samples, info.dps + i, 1);

  info.best_bits =
      LOG2BUFFER(info.sampleptrs[info.nterms], wps->wphdr.block_samples, 0) * 1;
...
void execute_mono(WavpackContext *wpc, int32_t *samples, int no_history,
  ...
  if (wps->num_passes > 1 && (wps->wphdr.flags & HYBRID_FLAG)) {
    ...
    decorr_mono_pass(samples, temp_buffer[0],
                     num_samples > 2048 ? 2048 : num_samples, &temp_decorr_pass,
                     -1);

    reverse_mono_decorr(&temp_decorr_pass);
    decorr_mono_pass(samples, temp_buffer[0], num_samples, &temp_decorr_pass,
                     1);
    CLEAR(temp_decorr_pass);
    temp_decorr_pass.delta = 2;
    ...
    decorr_mono_pass(temp_buffer[0], temp_buffer[1], num_samples,
                     &temp_decorr_pass, 1);
  ...
  for (pi = 0; pi < wps->num_passes;) {
    ...
    while (1) {
      ...
      for (j = 0; j < nterms; ++j) {
        ...
        decorr_mono_pass(temp_buffer[j & 1], temp_buffer[~j & 1], num_samples,
                         &temp_decorr_pass, 1);
      }
