...
static int64_t raw_get_pos(void *id) { return 0; }

static int raw_set_pos_abs(void *id, int64_t pos) { return 0; }

static int raw_set_pos_rel(void *id, int64_t delta, int mode) { return 0; }
...
static int64_t raw_get_length(void *id) { return 0; }

static int raw_can_seek(void *id) { return 0; }
...
static WavpackStreamReader64 raw_reader = {
    raw_read_bytes,  raw_write_bytes, raw_get_pos,
    raw_set_pos_abs, raw_set_pos_rel, raw_push_back_byte,
    raw_get_length,  raw_can_seek,    NULL,
    raw_close_stream};
...
WavpackContext *WavpackOpenRawDecoder(void *main_data, int32_t main_size,
  ...
  // if the WavPack data does not contain headers we assume Matroska-style
  // storage and recreate the missing headers

  if (strncmp(main_data, "wvpk", 4)) {
    uint32_t multiple_blocks = 0, block_size, block_samples = 0, wphdr_flags,
             crc;
    ...
    while (main_bytes >= 12) {
      ...
      if (block_size > main_bytes) {
        if (error)
          strcpy(error, "main block overran available data!");
      ...
      raw_wv->segments =
          realloc(raw_wv->segments, sizeof(RawSegment) * raw_wv->num_segments);
      raw_wv->segments[msi].dptr = raw_wv->segments[msi].sptr =
          (unsigned char *)wphdr;
      raw_wv->segments[msi].eptr =
          raw_wv->segments[msi].dptr + sizeof(WavpackHeader);
      ...
      if (corr_data && corr_bytes >= 4) {
        ...
        if (block_size > corr_bytes) {
          if (error)
            strcpy(error, "correction block overran available data!");
          raw_close_stream(raw_wv);
          raw_close_stream(raw_wvc);
          return NULL;
        }
        ...
        raw_wvc->segments = realloc(raw_wvc->segments,
                                    sizeof(RawSegment) * raw_wvc->num_segments);
        raw_wvc->segments[csi].dptr = raw_wvc->segments[csi].sptr =
            (unsigned char *)wphdr;
        raw_wvc->segments[csi].eptr =
            raw_wvc->segments[csi].dptr + sizeof(WavpackHeader);
    ...
    if (main_bytes || (corr_data && corr_bytes)) {
      if (error)
        strcpy(error, "leftover multiblock data!");
      raw_close_stream(raw_wv);
      raw_close_stream(raw_wvc);
      return NULL;
    }
  ...
  return WavpackOpenFileInputEx64(&raw_reader, raw_wv, raw_wvc, error,
                                  flags | OPEN_STREAMING | OPEN_NO_CHECKSUM,
                                  norm_offset);
}

// Return the number of samples represented by the current (and in the raw case,
// only) frame.
