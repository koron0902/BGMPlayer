...
static void best_floating_line(short *values, int num_values, double *initial_y,
                               double *final_y, short *max_error);
...
void dynamic_noise_shaping(WavpackContext *wpc, int32_t *buffer,
  ...
  if (!wps->num_terms && sample_count > 8) {
    if (flags & MONO_DATA)
      ...
      for (bptr = buffer + (sample_count - 3) * 2 + 1, sc = sample_count - 2;
           sc--;) {
  ...
  if (sample_count > wps->dc.shaping_samples) {
    ...
    if (flags & MONO_DATA)
      ...
      while (sc--) {
        ...
        *swptr++ = (ap->weight_A + ap->weight_B < 512)
                       ? 1024
                       : 1536 - ap->weight_A - ap->weight_B;
      }
  ...
  if (wpc->wvc_flag) {
    ...
    best_floating_line(wps->dc.shaping_data, sample_count, &initial_y, &final_y,
                       &max_error);
    ...
    if (initial_y < -512)
      initial_y = -512;
    else if (initial_y > 1024)
      initial_y = 1024;

    if (final_y < -512)
      final_y = -512;
    else if (final_y > 1024)
      final_y = 1024;
#if 0
    ...
    if (wpc->wvc_flag) {
      wps->dc.shaping_acc[0] = wps->dc.shaping_acc[1] =
          (int32_t)floor(initial_y * 65536.0 + 0.5);

      wps->dc.shaping_delta[0] = wps->dc.shaping_delta[1] = (int32_t)floor(
          (final_y - initial_y) / (sample_count - 1) * 65536.0 + 0.5);
...
// Given an array of integer data (in shorts), find the linear function that
// most closely represents it (based on minimum sum of absolute errors). This is
// returned as the double precision initial & final Y values of the best-fit
// line. The function can also optionally compute and return a maximum error
// value (as a short). Note that the ends of the resulting line may fall way
// outside the range of input values, so some sort of clipping may be needed.

static void best_floating_line(short *values, int num_values, double *initial_y,
                               double *final_y, short *max_error) {
  double left_sum = 0.0, right_sum = 0.0, center_x = (num_values - 1) / 2.0,
         center_y, m;
  int i;
