...
#define DECORR_STEREO_PASS_CONT_AVAILABLE                                      \
  unpack_cpu_has_feature_x86(CPU_FEATURE_MMX)
...
extern void DECORR_STEREO_PASS_CONT(struct decorr_pass *dpp, int32_t *buffer,
                                    int32_t sample_count, int32_t long_math);
extern void DECORR_MONO_PASS_CONT(struct decorr_pass *dpp, int32_t *buffer,
                                  int32_t sample_count, int32_t long_math);
#endif
...
static void decorr_stereo_pass(struct decorr_pass *dpp, int32_t *buffer,
                               int32_t sample_count);
static void decorr_mono_pass(struct decorr_pass *dpp, int32_t *buffer,
                             int32_t sample_count);
static void fixup_samples(WavpackContext *wpc, int32_t *buffer,
  ...
  if (!wps->block2buff && (flags & MONO_DATA)) {
    ...
    if (sample_count < 16)
      ...
      for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
        ...
        DECORR_MONO_PASS_CONT(dpp, buffer + pre_samples,
                              sample_count - pre_samples,
  ...
  /////////////// handle lossless or hybrid lossy stereo data ///////////////
    ...
    if (sample_count < 16 || !DECORR_STEREO_PASS_CONT_AVAILABLE) {
      ...
      for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
        ...
        DECORR_STEREO_PASS_CONT(dpp, buffer + pre_samples * 2,
                                sample_count - pre_samples,
