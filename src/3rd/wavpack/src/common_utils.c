...
const uint32_t sample_rates[] = {6000,  8000,  9600,  11025, 12000,
  ...
  if (wpc) {
    ...
    if (wpc->config.flags & (CONFIG_HIGH_FLAG | CONFIG_VERY_HIGH_FLAG)) {
      ...
      if ((wpc->config.flags & CONFIG_VERY_HIGH_FLAG) ||
          (wpc->streams && wpc->streams[0] &&
           wpc->streams[0]->wphdr.version < 0x405))
        mode |= MODE_VERY_HIGH;
    }
    ...
    if ((wpc->config.flags & CONFIG_HYBRID_FLAG) &&
        (wpc->config.flags & CONFIG_DYNAMIC_SHAPING) && wpc->streams &&
...
// QMODE_BIG_ENDIAN        0x1     // big-endian data format (opposite of WAV
// format) QMODE_SIGNED_BYTES      0x2     // 8-bit audio data is signed
// (opposite of WAV format) QMODE_UNSIGNED_WORDS    0x4     // audio data (other
// than 8-bit) is unsigned (opposite of WAV format) QMODE_REORDERED_CHANS   0x8
// // source channels were not Microsoft order, so they were reordered
// QMODE_DSD_LSB_FIRST     0x10    // DSD bytes, LSB first (most Sony .dsf
// files) QMODE_DSD_MSB_FIRST     0x20    // DSD bytes, MSB first (Philips .dff
// files) QMODE_DSD_IN_BLOCKS     0x40    // DSD data is blocked by channels
// (Sony .dsf only)
...
double WavpackGetInstantBitrate(WavpackContext *wpc) {
  ...
  if (wpc && wpc->streams && wpc->streams[0] &&
      wpc->streams[0]->wphdr.block_samples) {
    double output_time = (double)wpc->streams[0]->wphdr.block_samples /
...
// This function allows retrieving the Core Audio File channel layout, many of
...
// addition to the layout tag, this function returns the reordering string (if
// stored in the file) to allow the unpacker to reorder the channels back to the
// specified layout (if it wants to restore the CAF order). The number of
// channels in the layout is determined from the lower nybble of the layout word
// (and should probably match the number of channels in the file), and if a
// reorder string is requested then that much space must be allocated. Note that
// all the reordering is actually done outside of this library, and that if
// reordering is done then the appropriate qmode bit will be set.
//
// Note: Normally this function would not be used by an application unless it
// specifically wanted to restore a non-standard channel order (to check an MD5,
// for example) or obtain the Core Audio channel layout ID. For simple file
// decoding for playback, the channel_mask should provide all the information
// required unless there are non-Microsoft channels involved, in which case
// WavpackGetChannelIdentities() will provide the identities of the other
// channels (if they are known).
...
// This function provides the identities of ALL the channels in the file,
// including the standard Microsoft channels (which come first, in order, and
// are numbered 1-18) and also any non-Microsoft channels (which can be in any
// order and have values from 33-254). The value 0x00 is invalid and 0xFF
// indicates an "unknown" or "unnassigned" channel. The string is NULL
// terminated so the caller must supply enough space for the number of channels
// indicated by WavpackGetNumChannels(), plus one.
//
// Note that this function returns the actual order of the channels in the
// Wavpack file (i.e., the order returned by WavpackUnpackSamples()). If the
// file includes a "reordering" string because the source file was not in
// Microsoft order that is NOT taken into account here and really only needs to
// be considered if doing an MD5 verification or if it's required to restore the
// original order/file (like wvunpack does).

void WavpackGetChannelIdentities(WavpackContext *wpc,
...
// For local use only. Install a callback to be executed when WavpackCloseFile()
// is called, usually used to dump some statistics accumulated during encode or
// decode.
...
uint32_t WavpackGetSampleRate(WavpackContext *wpc) {
  return wpc ? (wpc->dsd_multiplier
                    ? wpc->config.sample_rate * wpc->dsd_multiplier
                    : wpc->config.sample_rate)
             : 44100;
}

// Returns the native sample rate of the specified WavPack file
// (provides the native rate for DSD files rather than the "byte" rate that's
// used for
//   seeking, duration, etc. and would generally be used just for user facing
//   reports)

uint32_t WavpackGetNativeSampleRate(WavpackContext *wpc) {
  return wpc ? (wpc->dsd_multiplier
                    ? wpc->config.sample_rate * wpc->dsd_multiplier * 8
                    : wpc->config.sample_rate)
             : 44100;
}
...
int WavpackGetReducedChannels(WavpackContext *wpc) {
  if (wpc)
    return wpc->reduced_channels ? wpc->reduced_channels
...
void WavpackLittleEndianToNative(void *data, char *format) {
  ...
  while (*format) {
    switch (*format) {
    case 'D':
      temp = cp[0] + ((int64_t)cp[1] << 8) + ((int64_t)cp[2] << 16) +
             ((int64_t)cp[3] << 24) + ((int64_t)cp[4] << 32) +
             ((int64_t)cp[5] << 40) + ((int64_t)cp[6] << 48) +
             ((int64_t)cp[7] << 56);
      *(int64_t *)cp = temp;
      cp += 8;
      break;
    ...
    case 'L':
      temp = cp[0] + ((int32_t)cp[1] << 8) + ((int32_t)cp[2] << 16) +
             ((int32_t)cp[3] << 24);
...
void WavpackBigEndianToNative(void *data, char *format) {
  ...
  while (*format) {
    switch (*format) {
    case 'D':
      temp = cp[7] + ((int64_t)cp[6] << 8) + ((int64_t)cp[5] << 16) +
             ((int64_t)cp[4] << 24) + ((int64_t)cp[3] << 32) +
             ((int64_t)cp[2] << 40) + ((int64_t)cp[1] << 48) +
             ((int64_t)cp[0] << 56);
      *(int64_t *)cp = temp;
      cp += 8;
      break;
    ...
    case 'L':
      temp = cp[3] + ((int32_t)cp[2] << 8) + ((int32_t)cp[1] << 16) +
             ((int32_t)cp[0] << 24);
      *(int32_t *)cp = (int32_t)temp;
      cp += 4;
      break;
...
uint32_t WavpackGetLibraryVersion(void) {
  return (LIBWAVPACK_MAJOR << 16) | (LIBWAVPACK_MINOR << 8) |
         (LIBWAVPACK_MICRO << 0);
}
