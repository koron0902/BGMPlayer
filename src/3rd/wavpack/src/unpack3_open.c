...
WavpackContext *open_file3(WavpackContext *wpc, char *error) {
  ...
  if (wpc->reader->read_bytes(wpc->wv_in, &RiffChunkHeader,
  ...
  if (!strncmp(RiffChunkHeader.ckID, "RIFF", 4) &&
      !strncmp(RiffChunkHeader.formType, "WAVE", 4)) {

    if (wpc->open_flags & OPEN_WRAPPER) {
      wpc->wrapper_data =
          (unsigned char *)malloc(wpc->wrapper_bytes = sizeof(RiffChunkHeader));
    ...
    while (1) {
      ...
      if (wpc->reader->read_bytes(wpc->wv_in, &ChunkHeader,
        ...
        if (wpc->open_flags & OPEN_WRAPPER) {
          wpc->wrapper_data = (unsigned char *)realloc(
              wpc->wrapper_data, wpc->wrapper_bytes + sizeof(ChunkHeader));
          memcpy(wpc->wrapper_data + wpc->wrapper_bytes, &ChunkHeader,
                 sizeof(ChunkHeader));
        ...
        if (!strncmp(ChunkHeader.ckID, "fmt ", 4)) {
          ...
          if (ChunkHeader.ckSize < sizeof(wavhdr) ||
              wpc->reader->read_bytes(wpc->wv_in, &wavhdr, sizeof(wavhdr)) !=
                  sizeof(wavhdr)) {
            if (error)
              strcpy(error, "not a valid WavPack file!");
            return WavpackCloseFile(wpc);
            ...
            wpc->wrapper_data = (unsigned char *)realloc(
                wpc->wrapper_data, wpc->wrapper_bytes + sizeof(wavhdr));
            memcpy(wpc->wrapper_data + wpc->wrapper_bytes, &wavhdr,
                   sizeof(wavhdr));
          ...
          if (ChunkHeader.ckSize > sizeof(wavhdr)) {
            uint32_t bytes_to_skip =
                (ChunkHeader.ckSize + 1 - sizeof(wavhdr)) & ~1L;
            ...
            if (wpc->open_flags & OPEN_WRAPPER) {
              wpc->wrapper_data = (unsigned char *)realloc(
                  wpc->wrapper_data, wpc->wrapper_bytes + bytes_to_skip);
              wpc->reader->read_bytes(wpc->wv_in,
                                      wpc->wrapper_data + wpc->wrapper_bytes,
                                      bytes_to_skip);
  ...
  if (wavhdr.FormatTag != 1 || !wavhdr.NumChannels || wavhdr.NumChannels > 2 ||
      !wavhdr.SampleRate || wavhdr.BitsPerSample < 16 ||
      wavhdr.BitsPerSample > 24 || wavhdr.BlockAlign / wavhdr.NumChannels > 3 ||
      wavhdr.BlockAlign % wavhdr.NumChannels ||
  ...
  if (((char *)&wphdr)[8] == 2 &&
      (wpc->reader->read_bytes(wpc->wv_in, ((char *)&wphdr) + 10, 2) != 2)) {
    if (error)
      strcpy(error, "not a valid WavPack file!");
    return WavpackCloseFile(wpc);
  } else if (((char *)&wphdr)[8] == 3 &&
             (wpc->reader->read_bytes(wpc->wv_in, ((char *)&wphdr) + 10,
                                      sizeof(wphdr) - 10) !=
              sizeof(wphdr) - 10)) {
    if (error)
      strcpy(error, "not a valid WavPack file!");
  ...
  if (strncmp(wphdr.ckID, "wvpk", 4) || wphdr.version < 1 ||
      wphdr.version > 3) {
    if (error)
      strcpy(error, "not a valid WavPack file!");
    return WavpackCloseFile(wpc);
  ...
  if (wphdr.version == 3) {
    ...
    if (wphdr.flags & EXTREME_DECORR) {
      ...
      if ((wphdr.flags & NOT_STORED_FLAGS) ||
          ((wphdr.bits) && (((wphdr.flags & NEW_HIGH_FLAG) &&
  ...
  if (wpc->wvc_in && wphdr.version == 3 && wphdr.bits &&
