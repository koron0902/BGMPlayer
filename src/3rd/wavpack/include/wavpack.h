...
// These are the mask bit definitions for the metadata chunk id byte (see
// format.txt)
...
// The lower 8 bits of qmode indicate the use of new features in version 5 that
// (presently) only apply to Core Audio Files (CAF) and DSD files, but could
// apply to other things too. These flags are stored in the file and can be
// retrieved by a decoder that is aware of them, but the individual bits are
// meaningless to the library. If ANY of these bits are set then the MD5 sum is
// written with a new ID so that old decoders will not see it (because these
// features will cause the MD5 sum to be different and fail).

#define QMODE_BIG_ENDIAN 0x1 // big-endian data format (opposite of WAV format)
#define QMODE_SIGNED_BYTES                                                     \
  0x2 // 8-bit audio data is signed (opposite of WAV format)
#define QMODE_UNSIGNED_WORDS                                                   \
  0x4 // audio data (other than 8-bit) is unsigned (opposite of WAV format)
#define QMODE_REORDERED_CHANS                                                  \
  0x8 // source channels were not Microsoft order, so they were reordered
...
#define QMODE_DSD_IN_BLOCKS                                                    \
  0x40 // DSD data is blocked by channels (Sony .dsf only)
#define QMODE_DSD_AUDIO (QMODE_DSD_LSB_FIRST | QMODE_DSD_MSB_FIRST)

// The rest of the qmode word is reserved for the private use of the
// command-line programs and are ignored by the library (and not stored either).
// They really should not be defined here, but I thought it would be a good idea
// to have all the definitions together.

#define QMODE_ADOBE_MODE 0x100 // user specified Adobe mode
#define QMODE_NO_STORE_WRAPPER                                                 \
  0x200 // user specified to not store audio file wrapper (RIFF, CAFF, etc.)
#define QMODE_CHANS_UNASSIGNED                                                 \
  0x400 // user specified "..." in --channel-order option
#define QMODE_IGNORE_LENGTH                                                    \
  0x800 // user specified to ignore length in file header
#define QMODE_RAW_PCM                                                          \
  0x1000 // user specified raw PCM format (no header present)
...
WavpackContext *WavpackOpenRawDecoder(void *main_data, int32_t main_size,
                                      ...
                                      int16_t version, char *error, int flags,
                                      int norm_offset);
