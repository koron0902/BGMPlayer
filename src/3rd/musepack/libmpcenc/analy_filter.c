...
#define EXPR(c, x)                                                             \
  (c[0] * x[0] + c[1] * x[64] + c[2] * x[128] + c[3] * x[192] +                \
  ...
  *y++ = EXPR((Ci_opt + 120), (x + 0)) + EXPR((Ci_opt + 256), (x + 32));
  ...
  for (i = 0; i < 16; i++, c += 32, x += 4, y += 4) {
    y[0] = c[0] * x[0] + c[1] * x[64] + c[2] * x[128] + c[3] * x[192] +
           c[4] * x[256] + c[5] * x[320] + c[6] * x[384] + c[7] * x[448];
    y[1] = c[8] * x[1] + c[9] * x[65] + c[10] * x[129] + c[11] * x[193] +
           c[12] * x[257] + c[13] * x[321] + c[14] * x[385] + c[15] * x[449];
    y[2] = c[16] * x[2] + c[17] * x[66] + c[18] * x[130] + c[19] * x[194] +
           c[20] * x[258] + c[21] * x[322] + c[22] * x[386] + c[23] * x[450];
    y[3] = c[24] * x[3] + c[25] * x[67] + c[26] * x[131] + c[27] * x[195] +
...
static void Matrixing(const int MaxBand, const float *mi, const float *y,
                      float *samples) {
  int i;
#ifdef FASTER
  for (i = 0; i <= MaxBand; i++, mi += 32,
      samples += 72) { // 144 = sizeof(SubbandFloatTyp)/sizeof(float)
    samples[0] =
        y[0] + mi[1] * y[1] + mi[2] * y[2] + mi[3] * y[3] + mi[4] * y[4] +
  ...
  for (i = 0; i <= MaxBand; i++, mi += 32,
      samples += 72) { // 144 = sizeof(SubbandFloatTyp)/sizeof(float)
    samples[0] = y[16] + mi[1] * (y[15] + y[17]) + mi[2] * (y[14] + y[18]) +
...
// Analysis-Filterbank
void Analyse_Filter(const PCMDataTyp *in, SubbandFloatTyp *out,
                    const int MaxBand) {
#ifdef FASTER
  float Y_L[32];
  float Y_R[32];
...
void Analyse_Init(float Left, float Right, SubbandFloatTyp *out,
                  const int MaxBand) {
#ifdef FASTER
  float Y_L[32];
  float Y_R[32];
