 ...
 * of bytes.
 */
static void FLAC__MD5Update(FLAC__MD5Context *ctx, FLAC__byte const *buf,
                            unsigned len) {
  FLAC__uint32 t;
 ...
 */
static void format_input_(FLAC__multibyte *mbuf,
                          const FLAC__int32 *const signal[], unsigned channels,
                          unsigned samples, unsigned bytes_per_sample) {
...
FLAC__bool FLAC__MD5Accumulate(FLAC__MD5Context *ctx,
                               ...
                               unsigned channels, unsigned samples,
                               unsigned bytes_per_sample) {
  const size_t bytes_needed =
      (size_t)channels * (size_t)samples * (size_t)bytes_per_sample;
  ...
  format_input_(&ctx->internal_buf, signal, channels, samples,
                bytes_per_sample);
