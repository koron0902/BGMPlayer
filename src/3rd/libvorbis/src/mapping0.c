...
static int mapping0_forward(vorbis_block *vb) {
  ...
  for (i = 0; i < vi->channels; i++) {
    ...
    for (j = 1; j < n - 1; j += 2) {
      ...
      temp = logfft[(j + 1) >> 1] = scale_dB + .5f * todB(&temp) + .345; /* +
                                                ...
                                                todB estimation used on IEEE 754
                                                compliant machines had a bug
                                                that returned dB values about a
                                                third of a decibel too high. The
                                                bug was harmless because tunings
                                                implicitly took that into
                                                account.  However, fixing the
                                                bug in the estimator requires
                                                changing all the tunings as
                                                well. For now, it's easier to
                                                sync things back up here, and
                                                recalibrate the tunings in the
                                                next major model upgrade. */
...

  {
    ...
    for (i = 0; i < vi->channels; i++) {
      ...
      floor_posts[i] =
          _vorbis_block_alloc(vb, PACKETBLOBS * sizeof(**floor_posts));
      ...
      _vp_offset_and_mix(psy_look, noise, tone, 1, logmask, mdct, logmdct);
      ...
      floor_posts[i][PACKETBLOBS / 2] =
          floor1_fit(vb, b->flr[info->floorsubmap[submap]], logmdct, logmask);
      ...
      if (vorbis_bitrate_managed(vb) && floor_posts[i][PACKETBLOBS / 2]) {
        ...
        _vp_offset_and_mix(psy_look, noise, tone, 2, logmask, mdct, logmdct);
        ...
        _vp_offset_and_mix(psy_look, noise, tone, 0, logmask, mdct, logmdct);
        ...
        for (k = 1; k < PACKETBLOBS / 2; k++)
          floor_posts[i][k] = floor1_interpolate_fit(
              vb, b->flr[info->floorsubmap[submap]], floor_posts[i][0],
              floor_posts[i][PACKETBLOBS / 2], k * 65536 / (PACKETBLOBS / 2));
        for (k = PACKETBLOBS / 2 + 1; k < PACKETBLOBS - 1; k++)
          floor_posts[i][k] = floor1_interpolate_fit(
              vb, b->flr[info->floorsubmap[submap]],
              floor_posts[i][PACKETBLOBS / 2], floor_posts[i][PACKETBLOBS - 1],
...

  {
    ...
    for (k = (vorbis_bitrate_managed(vb) ? 0 : PACKETBLOBS / 2);
      ...
      _vp_couple_quantize_normalize(
          k, &ci->psy_g_param, psy_look, info, gmdct, iwork, nonzero,
          ci->psy_g_param.sliding_lowpass[vb->W][k], vi->channels);
      ...
      for (i = 0; i < info->submaps; i++) {
        ...
        classifications = _residue_P[ci->residue_type[resnum]]->class(
            vb, b->residue[resnum], couple_bundle, zerobundle, ch_in_bundle);
        ...
        _residue_P[ci->residue_type[resnum]]->forward(
            opb, vb, b->residue[resnum], couple_bundle, zerobundle,
            ch_in_bundle, classifications, i);
      }
