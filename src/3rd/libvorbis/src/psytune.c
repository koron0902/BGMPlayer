  ...
  while (!eos) {
    ...
    if (bytes != 0) {
      ...
      for (i = 0; i < framesize; i++) {
        pcm[0][i] = ((buffer[i * 4 + 1] << 8) | (0x00ff & (int)buffer[i * 4])) /
                    32768.f;
        pcm[1][i] =
            ((buffer[i * 4 + 3] << 8) | (0x00ff & (int)buffer[i * 4 + 2])) /
            32768.f;
      }
      ...
      for (i = 0; i < 2; i++) {
        ...
        _vp_compute_mask(&p_look, pg_look, i, fft, logmdct, mask, ampmax,
                         local_ampmax[i], framesize / 2);
...

        {
          ...
          nonzero[i] =
              _floor_P[1]->forward(&vb, floor_look, mdct, logmdct, mask, logmax,

                                   flr[i]);
        }

        _vp_remove_floor(&p_look, pg_look, logmdct, mdct, flr[i], pcm[i],
                         local_ampmax[i]);
