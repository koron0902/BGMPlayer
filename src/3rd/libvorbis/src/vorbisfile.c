...
#define READSIZE                                                               \
  2048 /* a smaller read size is needed for low-rate streaming. */
...
static ogg_int64_t _get_prev_page(OggVorbis_File *vf, ogg_int64_t begin,
                                  ogg_page *og) {
...
static int _fetch_headers(OggVorbis_File *vf, vorbis_info *vi,
                          vorbis_comment *vc, long **serialno_list,
                          int *serialno_n, ogg_page *og_ptr) {
  ogg_page og;
  ogg_packet op;
  int i, ret;
  int allbos = 0;
...
static int _bisect_forward_serialno(OggVorbis_File *vf, ogg_int64_t begin,
                                    ogg_int64_t searched, ogg_int64_t end,
                                    ogg_int64_t endgran, int endserial,
                                    long *currentno_list, int currentnos,
                                    long m) {
  ...
  */
  ...
  if (_lookup_serialno(endserial, currentno_list, currentnos)) {
    ...
    while (endserial != serialno) {
      endserial = serialno;
      searched = _get_prev_page_serial(vf, searched, currentno_list, currentnos,
                                       &endserial, &endgran);
    }
    ...
    ret = _fetch_headers(vf, &vi, &vc, &next_serialno_list, &next_serialnos,
                         NULL);
    if (ret)
      return (ret);
    ...
    ret =
        _bisect_forward_serialno(vf, next, vf->offset, end, endgran, endserial,
...
static int _open_seekable2(OggVorbis_File *vf) {
  ...
  end = _get_prev_page_serial(vf, vf->end, vf->serialnos + 2, vf->serialnos[1],
                              &endserial, &endgran);
  if (end < 0)
    return (end);
...
static int _fetch_and_process_packet(OggVorbis_File *vf, ogg_packet *op_in,
                                     int readp, int spanp) {
  ogg_page og;
...
int ov_open_callbacks(void *f, OggVorbis_File *vf, const char *initial,
                      long ibytes, ov_callbacks callbacks) {
...
int ov_test_callbacks(void *f, OggVorbis_File *vf, const char *initial,
                      long ibytes, ov_callbacks callbacks) {
...
long ov_streams(OggVorbis_File *vf) { return vf->links; }
...
int ov_raw_seek(OggVorbis_File *vf, ogg_int64_t pos) {
  ...
  if (vf->ready_state >= STREAMSET) {
    if (pos < vf->offsets[vf->current_link] ||
        pos >= vf->offsets[vf->current_link + 1])
...

  {
    ...
    while (begin < end) {
      ...
      if (end - begin < CHUNKSIZE) {
        ...
        bisect = begin +
                 (ogg_int64_t)((double)(target - begintime) * (end - begin) /
                               (endtime - begintime)) -
                 CHUNKSIZE;
    ...
    if (best == -1) {
      ...
      if (got_page && begin == vf->dataoffsets[link] &&
      ...
      /* Bisection found our page. seek to it, update pcm offset. Easier case
         than raw_seek, don't keep packets preceding granulepos. */
        ...
        if (result == 0) {
          ...
          while (result > vf->dataoffsets[link]) {
            ...
            if (ogg_page_serialno(&og) == vf->current_serialno &&
                (ogg_page_granulepos(&og) > -1 || !ogg_page_continued(&og))) {
...
long ov_read_filter(OggVorbis_File *vf, char *buffer, int length,
                    ...
                    void (*filter)(float **pcm, long channels, long samples,
                                   void *filter_param),
                    void *filter_param) {
  int i, j;
...
long ov_read(OggVorbis_File *vf, char *buffer, int length, int bigendianp,
  ...
  return ov_read_filter(vf, buffer, length, bigendianp, word, sgned, bitstream,
                        NULL, NULL);
}
...
static void _ov_splice(float **pcm, float **lappcm, int n1, int n2, int ch1,
                       int ch2, const float *w1, const float *w2) {
  int i, j;
  const float *w = w1;
  int n = n1;
...
static void _ov_getlap(OggVorbis_File *vf, vorbis_info *vi,
                       vorbis_dsp_state *vd, float **lappcm, int lapsize) {
  int lapcount = 0, i;
  float **pcm;
