...
static int accumulate_fit(const float *flr, const float *mdct, int x0, int x1,
                          lsfit_acc *a, int n, vorbis_info_floor1 *info) {
  long i;

  int xa = 0, ya = 0, x2a = 0, y2a = 0, xya = 0, na = 0, xb = 0, yb = 0,
...
static int fit_line(lsfit_acc *a, int fits, int *y0, int *y1,
  ...
  for (i = 0; i < fits; i++) {
    double weight =
        (a[i].bn + a[i].an) * info->twofitweight / (a[i].an + 1) + 1.;
...
static int inspect_error(int x0, int x1, int y0, int y1, const float *mask,
                         const float *mdct, vorbis_info_floor1 *info) {
  int dy = y1 - y0;
  int adx = x1 - x0;
  int ady = abs(dy);
  int base = dy / adx;
...
int *floor1_interpolate_fit(vorbis_block *vb, vorbis_look_floor1 *look, int *A,
                            int *B, int del) {
  ...
  if (A && B) {
    ...
    for (i = 0; i < posts; i++) {
      output[i] =
          ((65536 - del) * (A[i] & 0x7fff) + del * (B[i] & 0x7fff) + 32768) >>
          16;
  ...
  if (post) {
    ...
    for (i = 2; i < posts; i++) {
      ...
      if ((post[i] & 0x8000) || (predicted == post[i])) {
        ...
        int headroom =
            (look->quant_q - predicted < predicted ? look->quant_q - predicted
                                                   : predicted);
    ...
    for (i = 0, j = 2; i < info->partitions; i++) {
      ...
      if (csubbits) {
        int maxval[8] = {0, 0, 0, 0, 0,
                         0, 0, 0}; /* gcc's static analysis
...
#ifdef TRAIN_FLOOR1
        {
          FILE *of;
          char buffer[80];
          sprintf(buffer, "line_%dx%ld_class%d.vqd", vb->pcmend / 2, posts - 2,
                  class);
          of = fopen(buffer, "a");
          fprintf(of, "%d\n", cval);
          fclose(of);
      ...
      for (k = 0; k < cdim; k++) {
        ...
        if (book >= 0) {
...
#ifdef TRAIN_FLOOR1
          {
            FILE *of;
            char buffer[80];
            sprintf(buffer, "line_%dx%ld_%dsub%d.vqd", vb->pcmend / 2,
                    posts - 2, class, bookas[k]);
            of = fopen(buffer, "a");
            fprintf(of, "%d\n", out[j + k]);
            fclose(of);
          }
  ...
  if (oggpack_read(&vb->opb, 1) == 1) {
    ...
    for (i = 0, j = 2; i < info->partitions; i++) {
      ...
      for (k = 0; k < cdim; k++) {
        ...
        cval >>= csubbits;
        if (book >= 0) {
          if ((fit_value[j + k] = vorbis_book_decode(books + book, &vb->opb)) ==
              -1)
            goto eop;
        } else {
          fit_value[j + k] = 0;
