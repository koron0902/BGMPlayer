...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS], float binHz,
                                  int n, float center_boost,
  ...
  for (i = 0; i < P_BANDS; i++) {
    ...
    for (j = 0; j < P_LEVELS; j++) {
      attenuate_curve(workc[i][j], curveatt_dB[i] + 100. -
                                       (j < 2 ? 2 : j) * 10. - P_LEVEL_0);
...
void _vp_psy_init(vorbis_look_psy *p, vorbis_info_psy *vi,
  ...
  for (i = 0; i < n; i++)
    p->octave[i] =
        toOC((i + .25f) * .5 * rate / n) * (1 << (p->shiftoc + 1)) + .5f;
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < P_NOISECURVES; j++)
      p->noiseoffset[j][i] = p->vi->noiseoff[j][inthalfoc] * (1. - del) +
...
static void seed_curve(float *seed, const float **curves, float amp, int oc,
                       int n, int linesper, float dBoffset) {
  int i, post1;
  int seedptr;
...
static void seed_loop(vorbis_look_psy *p, const float ***curves, const float *f,
                      const float *flr, float *seed, float specmax) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (max + 6.f > flr[i]) {
      ...
      seed_curve(seed, curves[oc], max, p->octave[i] - p->firstoc,
                 p->total_octave_lines, p->eighth_octave_lines, dBoffset);
    }
  }
}
...
static void bark_noise_hybridmp(int n, const long *b, const float *f,
                                float *noise, const float offset,
                                const int fixed) {
...
void _vp_noisemask(vorbis_look_psy *p, float *logmdct, float *logmask) {
  ...
  bark_noise_hybridmp(n, p->bark, work, logmask, 0., p->vi->noisewindowfixed);
...
void _vp_tonemask(vorbis_look_psy *p, float *logfft, float *logmask,
...
void _vp_offset_and_mix(vorbis_look_psy *p, float *noise, float *tone,
                        int offset_select, float *logmask, float *mdct,
                        float *logmdct) {
  int i, n = p->n;
...
static void flag_lossless(int limit, float prepoint, float postpoint,
                          float *mdct, float *floor, int *flag, int i, int jn) {
  int j;
  for (j = 0; j < jn; j++) {
...
static float noise_normalize(vorbis_look_psy *p, int limit, float *r, float *q,
                             float *f, int *flags, float acc, int i, int n,
                             int *out) {
  ...
  for (; j < n; j++) {
    if (!flags || !flags[j]) { /* can't noise norm elements that have
      ...
      if (ve < .25f && (!flags || j >= limit - i)) {
        ...
        /* For now: no acc adjustment for nonzero quantization.  populate *out
         * and q as this value is final. */
        if (r[j] < 0)
          out[j] = -rint(sqrt(ve));
        else
          out[j] = rint(sqrt(ve));
...
void _vp_couple_quantize_normalize(int blobno, vorbis_info_psy_global *g,
                                   vorbis_look_psy *p, vorbis_info_mapping0 *vi,
                                   float **mdct, int **iwork, int *nonzero,
                                   int sliding_lowpass, int ch) {
  ...
  for (i = 0; i < n; i += partition) {
    ...
    for (k = 0; k < ch; k++) {
      ...
      if (nz[k]) {
        ...
        flag_lossless(limit, prepoint, postpoint, &mdct[k][i], floor[k],
                      flag[k], i, jn);
        ...
        acc[track] = noise_normalize(p, limit, raw[k], quant[k], floor[k], NULL,
                                     acc[track], i, jn, iout);
    ...
    for (step = 0; step < vi->coupling_steps; step++) {
      ...
      if (nz[Mi] || nz[Ai]) {
        ...
        acc[track] = noise_normalize(p, limit, raw[Mi], quant[Mi], floor[Mi],
                                     flag[Mi], acc[track], i, jn, iM);
        track++;
      }
    }
  }
