...
static const vorbis_info_mode _mode_template[2] = {{0, 0, 0, 0}, {1, 0, 0, 1}};
...
static void
vorbis_encode_floor_setup(vorbis_info *vi, int s,
                          const static_codebook *const *const *const books,
                          const vorbis_info_floor1 *in, const int *x) {
  int i, k, is = s;
...
static void vorbis_encode_psyset_setup(vorbis_info *vi, double s,
                                       ...
                                       const double *nn_thresh, int block) {
  codec_setup_info *ci = vi->codec_setup;
...
static void vorbis_encode_noisebias_setup(vorbis_info *vi, double s, int block,
                                          const int *suppress, const noise3 *in,
  ...
  for (j = 0; j < P_NOISECURVES; j++)
    for (i = 0; i < P_BANDS; i++)
      p->noiseoff[j][i] =
          in[is].data[j][i] * (1. - ds) + in[is + 1].data[j][i] * ds;
...
static void vorbis_encode_residue_setup(vorbis_info *vi, int number, int block,
                                        const vorbis_residue_template *res) {
    ...
    if (ci->hi.managed) {
      ...
      for (i = 0; i < r->partitions; i++) {
        for (k = 0; k < 4; k++) {
          if (res->books_base_managed->books[i][k]) {
            int bookid =
                book_dup_or_new(ci, res->books_base_managed->books[i][k]);
            r->booklist[booklist++] = bookid;
            ci->book_param[bookid] =
                (static_codebook *)res->books_base_managed->books[i][k];
          }
        }
      }
      ...
      for (i = 0; i < r->partitions; i++) {
        for (k = 0; k < 4; k++) {
          if (res->books_base->books[i][k]) {
            ...
            r->booklist[booklist++] = bookid;
            ci->book_param[bookid] =
                (static_codebook *)res->books_base->books[i][k];
          }
        }
      }
    }
    ...
    if (ci->residue_type[number] == 2) {
      ...
      for (i = 0; i < ci->maps && ch == 0; i++) {
        ...
        for (j = 0; j < mi->submaps && ch == 0; j++)
          if (mi->residuesubmap[j] ==
              number) /* we found a submap referencing theis residue backend */
            for (k = 0; k < vi->channels; k++)
              if (mi->chmuxlist[k] ==
                  j) /* this channel belongs to the submap */
                ch++;
      }

      r->end = (int)((freq / nyq * blocksize * ch) / r->grouping +
...
static const void *get_setup_template(long ch, long srate, double req,
                                      int q_or_bitrate, double *base_setting) {
  int i = 0, j;
  if (q_or_bitrate)
    req /= ch;
  ...
  while (setup_list[i]) {
    if (setup_list[i]->coupling_restriction == -1 ||
      ...
      if (srate >= setup_list[i]->samplerate_min_restriction &&
        ...
        const double *map = (q_or_bitrate ? setup_list[i]->rate_mapping
...
int vorbis_encode_setup_init(vorbis_info *vi) {
  ...
  for (i = 0; i < setup->floor_mappings; i++)
    vorbis_encode_floor_setup(vi, hi->base_setting, setup->floor_books,
                              setup->floor_params,
                              setup->floor_mapping_list[i]);

  /* setup of [mostly] short block detection and stereo*/
  vorbis_encode_global_psych_setup(vi, hi->trigger_setting,
                                   setup->global_params, setup->global_mapping);
  vorbis_encode_global_stereo(vi, hi, setup->stereo_modes);

  /* basic psych setup and noise normalization */
  vorbis_encode_psyset_setup(
      vi, hi->base_setting, setup->psy_noise_normal_start[0],
      setup->psy_noise_normal_partition[0], setup->psy_noise_normal_thresh, 0);
  vorbis_encode_psyset_setup(
      vi, hi->base_setting, setup->psy_noise_normal_start[0],
      setup->psy_noise_normal_partition[0], setup->psy_noise_normal_thresh, 1);
  if (!singleblock) {
  ...
  vorbis_encode_tonemask_setup(vi, hi->block[i0].tone_mask_setting, 0,
                               setup->psy_tone_masteratt, setup->psy_tone_0dB,
  ...
  if (!singleblock) {
    vorbis_encode_noisebias_setup(
        ...
        setup->psy_noise_bias_trans, setup->psy_noiseguards, 0.);
    vorbis_encode_noisebias_setup(
        vi, hi->block[3].noise_bias_setting, 3, setup->psy_noise_dBsuppress,
        setup->psy_noise_bias_long, setup->psy_noiseguards, 0.);
  }
...
static void vorbis_encode_setup_setting(vorbis_info *vi, long channels,
                                        long rate) {
  int i, is;
  codec_setup_info *ci = vi->codec_setup;
  highlevel_encode_setup *hi = &ci->hi;
  ...
  hi->ath_floating_dB =
      setup->psy_ath_float[is] * (1. - ds) + setup->psy_ath_float[is + 1] * ds;
  hi->ath_absolute_dB =
      setup->psy_ath_abs[is] * (1. - ds) + setup->psy_ath_abs[is + 1] * ds;
...
int vorbis_encode_setup_vbr(vorbis_info *vi, long channels, long rate,
                            float quality) {
  codec_setup_info *ci;
...
int vorbis_encode_setup_managed(vorbis_info *vi, long channels, long rate,

                                long max_bitrate, long nominal_bitrate,
                                long min_bitrate) {
  ...
  hi->setup =
      get_setup_template(channels, rate, nominal_bitrate, 1, &hi->base_setting);
  if (!hi->setup)
    return OV_EIMPL;
...
int vorbis_encode_ctl(vorbis_info *vi, int number, void *arg) {
  if (vi) {
    ...
    case OV_ECTL_RATEMANAGE_GET: {
      ...
      struct ovectl_ratemanage_arg *ai = (struct ovectl_ratemanage_arg *)arg;
    ...
    case OV_ECTL_RATEMANAGE_HARD: {
      ...
      if (ai == NULL) {
        ...
        hi->bitrate_reservoir =
            ai->bitrate_hard_window * (hi->bitrate_max + hi->bitrate_min) * .5;
      }
    ...
    case OV_ECTL_RATEMANAGE2_GET: {
      struct ovectl_ratemanage2_arg *ai = (struct ovectl_ratemanage2_arg *)arg;
      if (ai == NULL)
        return OV_EINVAL;
    ...
    case OV_ECTL_RATEMANAGE2_SET: {
      ...
      if (ai == NULL) {
        ...
        if (ai->bitrate_limit_min_kbps > 0 && ai->bitrate_average_kbps > 0 &&
            ai->bitrate_limit_min_kbps > ai->bitrate_average_kbps)
          return OV_EINVAL;

        if (ai->bitrate_limit_max_kbps > 0 && ai->bitrate_average_kbps > 0 &&
            ai->bitrate_limit_max_kbps < ai->bitrate_average_kbps)
          return OV_EINVAL;

        if (ai->bitrate_limit_min_kbps > 0 && ai->bitrate_limit_max_kbps > 0 &&
    ...
    case OV_ECTL_COUPLING_SET: {
      ...
      new_template =
          get_setup_template(hi->coupling_p ? vi->channels : -1, vi->rate,
                             hi->req, hi->managed, &new_base);
      if (!hi->setup)
        return OV_EIMPL;
