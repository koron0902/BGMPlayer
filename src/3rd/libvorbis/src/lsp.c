...
void vorbis_lsp_to_curve(float *curve, int *map, int n, int ln, float *lsp,
  ...
  while (i < n) {
    ...
    q = frexp(p + q, &qexp);
    q = vorbis_fromdBlook(
        amp * vorbis_invsqlook(q) * vorbis_invsq2explook(qexp + m) - ampoffset);
...
static int Laguerre_With_Deflation(float *a, int ord, float *r) {
  ...
  for (m = ord; m > 0; m--) {
    ...
    while (1) {
      ...
      if (denom < 0)
        return (
            -1); /* complex root!  The LPC generator handed us a bad filter */
